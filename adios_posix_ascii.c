...
void adios_posix_ascii_open (struct adios_file_struct * fd
                            ,struct adios_method_struct * method
                            )
{
    char name [STR_LEN];
    ...
    else
    {
        if (fd->mode & adios_mode_append)
        {
            ...
            if (f->f == -1)
            {
                ...
                if (f->f == -1)
                {
                    fprintf (stderr, "adios_posix_ascii_open failed for "
                                     "base_path %s, name %s\n"
...
// need to handle overflow of allocatable space
            fprintf (stderr, "OVERFLOW in posix_ascii when writing %s\n", v->name);
...
static void adios_posix_ascii_do_write (struct adios_file_struct * fd
                                       ,struct adios_method_struct * method
                                       )
{
...
static void adios_posix_ascii_do_read (struct adios_file_struct * fd
                                      ,struct adios_method_struct * method
                                      )
{
    struct adios_File_data_struct * md = (struct adios_File_data_struct *)
    ...
    while ((element_size = br_get_next_element_specific (md->handle
                                                        ,posix_ascii_read_pre_fetch
                                                        ,posix_ascii_read_post_fetch
                                                        ,&data
                                                        ,&element
                                                        )
          ) != 0)
                             ...
                             )
{
    ...
    if (fd->mode & adios_mode_write)
        adios_posix_ascii_do_write (fd, method);

    if (fd->mode & adios_mode_read)
    {
        adios_posix_ascii_do_read (fd, method);
        struct adios_var_struct * v = fd->group->vars;
        while (v)
        {
            v->data = 0;
