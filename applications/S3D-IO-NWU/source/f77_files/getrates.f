C $Id: getrates.f,v 1.1.1.1.6.2 2006/04/04 18:35:32 rsankar Exp $
C Getrates for Dryer's CO/H2 
C Skeletal - H2O2 and its reactions are ignored.
C Gibbs lookup added.
c****************************************************************************
...
c****************************************************************************
      ...
      integer NC_H2, NC_O2, NC_O, NC_OH, NC_H2O, NC_H, NC_HO2, NC_CO
      integer NC_CO2, NC_HCO, NC_N2
...
C      end do
      call gibbsEnrg_all_dimT(tc,cgspl)
...
c   9)  O2 + H (+M) <=> HO2 (+M) 
      ...
      fcent = log10(2.0d-001 * exp(-9.999999999999999d+029 * tc) + 
     &  8.0d-001 * exp(-9.999999999999999d-031 * tc))
...
c  14)  O + CO (+M) <=> CO2 (+M) 
      ...
      rr_r = rr_f * exp(xik*otc) * prt
      ropl(14) = rr_f*cspl(NC_O)*cspl(NC_CO) - rr_r*cspl(NC_CO2)
      
c  15)  O2 + CO <=> O + CO2
      rr_f = 2.53d+012 * exp(-4.77d+004*ortc)
      xik = -cgspl(NC_O2) - cgspl(NC_CO) + cgspl(NC_O) + cgspl(NC_CO2)
      rr_r = rr_f * exp(xik*otc)
      ropl(15) = rr_f*cspl(NC_O2)*cspl(NC_CO) - rr_r*cspl(NC_O)
     &  *cspl(NC_CO2)
...
c  16)  HO2 + CO <=> OH + CO2
     ...
     &  
      rr_r = rr_f * exp(xik*otc)
      ropl(16) = rr_f*cspl(NC_HO2)*cspl(NC_CO) - rr_r*cspl(NC_OH)
     &  *cspl(NC_CO2)
...
c  17)  OH + CO <=> H + CO2
      ...
      rr_r = rr_f * exp(xik*otc)
      ropl(17) = rr_f*cspl(NC_OH)*cspl(NC_CO) - rr_r*cspl(NC_H)
     &  *cspl(NC_CO2)
...
c  18)  HCO + M <=> CO + H + M 
      ...
      rr_r = rr_f * exp(xik*otc) * oprt
      ropl(18) = rr_f*cspl(NC_HCO) - rr_r*cspl(NC_CO)*cspl(NC_H)
      ropl(18) = ropl(18) * thbctemp(4)
...
c  19)  O2 + HCO <=> HO2 + CO
     ...
     &  
      rr_r = rr_f * exp(xik*otc)
      ropl(19) = rr_f*cspl(NC_O2)*cspl(NC_HCO) - rr_r*cspl(NC_HO2)
     &  *cspl(NC_CO)
...
c  20)  H + HCO <=> H2 + CO
      ...
      rr_r = rr_f * exp(xik*otc)
      ropl(20) = rr_f*cspl(NC_H)*cspl(NC_HCO) - rr_r*cspl(NC_H2)
     &  *cspl(NC_CO)
...
c  21)  O + HCO <=> H + CO2
      ...
      rr_r = rr_f * exp(xik*otc)
      ropl(21) = rr_f*cspl(NC_O)*cspl(NC_HCO) - rr_r*cspl(NC_H)
     &  *cspl(NC_CO2)
...
c  1. H2
      wspl(NC_H2) = -ropl(2)-ropl(3)-ropl(5)+ropl(10)+ropl(20)
c  2. O2
      wspl(NC_O2) = -ropl(1)+ropl(6)-ropl(9)+ropl(10)+ropl(12)+ropl(13)
     &  -ropl(15)-ropl(19)
c  3. O
      wspl(NC_O) = ropl(1)-ropl(2)-ropl(4)-2.0*ropl(6)-ropl(7)-ropl(12)
     &  -ropl(14)+ropl(15)-ropl(21)
c  4. OH
      wspl(NC_OH) = ropl(1)+ropl(2)-ropl(3)+2.0*ropl(4)+ropl(7)-ropl(8)
     &  +2.0*ropl(11)+ropl(12)-ropl(13)+ropl(16)-ropl(17)
c  5. H2O
      wspl(NC_H2O) = ropl(3)-ropl(4)+ropl(8)+ropl(13)
c  6. H
      wspl(NC_H) = -ropl(1)+ropl(2)+ropl(3)+2.0*ropl(5)-ropl(7)-ropl(8)
     &  -ropl(9)-ropl(10)-ropl(11)+ropl(17)+ropl(18)-ropl(20)+ropl(21)
c  7. HO2
      wspl(NC_HO2) = ropl(9)-ropl(10)-ropl(11)-ropl(12)-ropl(13)
     &  -ropl(16)+ropl(19)
c  8. CO
      wspl(NC_CO) = -ropl(14)-ropl(15)-ropl(16)-ropl(17)+ropl(18)
     &  +ropl(19)+ropl(20)
c  9. CO2
      wspl(NC_CO2) = ropl(14)+ropl(15)+ropl(16)+ropl(17)+ropl(21)
c 10. HCO
      wspl(NC_HCO) = -ropl(18)-ropl(19)-ropl(20)-ropl(21)
c 11. N2
      wspl(NC_N2) = 0.0
