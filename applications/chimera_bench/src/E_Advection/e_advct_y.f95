SUBROUTINE e_advct_y( jmin, jmax, nn, ji_ray, jk_ray, i_radial )
!-----------------------------------------------------------------------
!
!    File:         e_advct_y
!    Module:       e_advct_y
...
!       energy grid as a result of the y-hydro step.
!
!    Subprograms called:
!  e_advct_y_v      : calculates neutrino "energy velocities" at the energy zone edges
...
!
!    Input arguments:
!  jmin             : minimum y-zone for which nu energy advection is to be calculated
!  jmax             : minimum y-§zone for which nu energy advection is to be calculated
...
!  rho(j)           : matter density of zone j before the y-hydro step (g/cm**3)
!  rhoa(j)          : matter density of zone j after the y-hydro step (g/cm**3)
...
!-----------------------------------------------------------------------
  ...
  CALL e_advct_y_v( j, nn, i_radial )
...
!  DO k = 1,nnugp(nn)
!    dndt_v(j,k,nn,ji_ray,jk_ray) = ncoefaa(j,k) * psi0_a(j,k,nn) - ncoefa(j,k) * psi0(j,k,nn)
!  END DO

!-----------------------------------------------------------------------
!  unujv(j,n)      : energy transferred to n-type neutrinos by advection
!                     in zone j (ergs)
!  unucrv(n,ji_ray,jk_ray) : total energy transferred to n-type neutrinos by
!                     advection (ergs)
...
!  unujv(j,nn,ji_ray,jk_ray)  = unujv(j,nn,ji_ray,jk_ray) + unuvt * dmrst(j)
!  unucrv(nn,ji_ray,jk_ray)   = unucrv(nn,ji_ray,jk_ray) + unuvt * dmrst(j)
!  dunujvdt(j,nn,ji_ray,jk_ray) = unuvt/dtnph
