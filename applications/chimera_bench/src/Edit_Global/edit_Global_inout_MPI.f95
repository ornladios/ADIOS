...
!
!    Include files:
!  numerical_module, array_module, kind_module
!  cycle_module, edit_module, eos_snc_x_module, nucbrn_module,
!  nu_dist_module, parallel_module, shock_module, t_cntrl_module
!
!-----------------------------------------------------------------------
...
USE edit_module, ONLY : nned_global, inted_global, ied_global_n, dt_global_ed1, &
& dt_global_ed2, ied_global_t
...
INTEGER, DIMENSION(nx,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z) :: irecv_buf ! receive buffer for gathering data from processors
...
REAL(KIND=double), DIMENSION(11,nx,ij_ray_dim,ik_ray_dim)                      :: send_buf1      ! send buffer for gathering data from processors
REAL(KIND=double), DIMENSION(11,nx,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z)    :: recv_buf1      ! receive buffer for gathering data from processors
REAL(KIND=double), DIMENSION(2,nx,nnu,ij_ray_dim,ik_ray_dim)                   :: send_buf2      ! send buffer for gathering data from processors
REAL(KIND=double), DIMENSION(2,nx,nnu,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z) :: recv_buf2      ! receive buffer for gathering data from processors
REAL(KIND=double), DIMENSION(nx,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z)       :: recv_buf_nx    ! receive buffer for gathering data from processors
REAL(KIND=double), DIMENSION(nx,nnu,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z)   :: recv_buf_nxnnu ! receive buffer for gathering data from processors
REAL(KIND=double), DIMENSION(nx,nnc,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z)   :: recv_buf_nnc   ! receive buffer for gathering data from processors
REAL(KIND=double), DIMENSION(4+nnc,nx,ij_ray_dim,ik_ray_dim)                   :: send_buf4     ! send buffer for gathering data from processors
REAL(KIND=double), DIMENSION(4+nnc,nx,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z) :: recv_buf4        ! receive buffer for gathering data from processors
...
sMD_in(imin:imax,1:ij_ray_dim,1:ik_ray_dim)                             &
&                            = aesv(imin+1:imax+1,3,1:ij_ray_dim,1:ik_ray_dim)
pMD_in(imin:imax,1:ij_ray_dim,1:ik_ray_dim)                             &
&                            = aesv(imin+1:imax+1,1,1:ij_ray_dim,1:ik_ray_dim)
gamMD_in(imin:imax,1:ij_ray_dim,1:ik_ray_dim)                           &
&                            = aesv(imin+1:imax+1,12,1:ij_ray_dim,1:ik_ray_dim)
...
send_buf1( 2,:,:,:)          = vMD_in  (:,:,:)
send_buf1( 3,:,:,:)          = wMD_in  (:,:,:)
send_buf1( 4,:,:,:)          = rhoMD_in(:,:,:)
send_buf1( 5,:,:,:)          = tMD_in  (:,:,:)
send_buf1( 6,:,:,:)          = sMD_in  (:,:,:)
send_buf1( 7,:,:,:)          = yeMD_in (:,:,:)
send_buf1( 8,:,:,:)          = dudt_nuc(:,:,:)
send_buf1( 9,:,:,:)          = dudt_nu (:,:,:)
send_buf1(10,:,:,:)          = pMD_in  (:,:,:)
send_buf1(11,:,:,:)          = gamMD_in(:,:,:)

!........x-velocity

c_gath_recv                  = 11 * nx * ij_ray_dim * ik_ray_dim
c_gath_send                  = 11 * nx * ij_ray_dim * ik_ray_dim

CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
CALL MPI_GATHER( send_buf1, c_gath_send, MPI_DOUBLE_PRECISION, recv_buf1, c_gath_recv, &
...
!-----------------------------------------------------------------------
  ...
  DO m = 0,n_proc-1
    ...
    DO k = 1,ik_ray_dim
      ...
      DO j = 1,ij_ray_dim
        jsk                      = mj * ij_ray_dim + j
        uMD       (1:nx,jsk,ksk) = recv_buf1( 1,1:nx,j,k,m+1)
        vMD       (1:nx,jsk,ksk) = recv_buf1( 2,1:nx,j,k,m+1)
        wMD       (1:nx,jsk,ksk) = recv_buf1( 3,1:nx,j,k,m+1)
        rhoMD     (1:nx,jsk,ksk) = recv_buf1( 4,1:nx,j,k,m+1)
        tMD       (1:nx,jsk,ksk) = recv_buf1( 5,1:nx,j,k,m+1)
        sMD       (1:nx,jsk,ksk) = recv_buf1( 6,1:nx,j,k,m+1)
        yeMD      (1:nx,jsk,ksk) = recv_buf1( 7,1:nx,j,k,m+1)
        dudt_nucMD(1:nx,jsk,ksk) = recv_buf1( 8,1:nx,j,k,m+1)
        dudt_nuMD (1:nx,jsk,ksk) = recv_buf1( 9,1:nx,j,k,m+1)
        pMD       (1:nx,jsk,ksk) = recv_buf1(10,1:nx,j,k,m+1)
        gamMD     (1:nx,jsk,ksk) = recv_buf1(11,1:nx,j,k,m+1)
...
DO k = 1,ik_ray_dim
  DO j = 1,ij_ray_dim
    CALL luminosity_MD( imin, imax, j, k, ij_ray_dim, ik_ray_dim, nx, &
&     nez, nnu, x_e_in, psi1_in, unu_in, dunu_in, lum_in )
    CALL e_rms_MD( imin, imax, j, k, ij_ray_dim, ik_ray_dim, nx, nez, &
&    nnu, psi0_in, psi1_in, unu_in, dunu_in, e_rms_stat_in, e_rms_trns_in )
  END DO ! j = 1,ij_ray_dim
END DO ! k = 1,ik_ray_dim
...
send_buf2(1,:,:,:,:)             = lum_in       (:,:,:,:)
send_buf2(2,:,:,:,:)             = e_rms_stat_in(:,:,:,:)

!-----------------------------------------------------------------------
!  Gather quantities
!-----------------------------------------------------------------------

c_gath_recv                  = 2 * nx * nnu * ij_ray_dim * ik_ray_dim
c_gath_send                  = 2 * nx * nnu * ij_ray_dim * ik_ray_dim
...
!-----------------------------------------------------------------------
  ...
  DO m = 0,n_proc-1
    ...
    DO k = 1,ik_ray_dim
      ...
      DO j = 1,ij_ray_dim
        jsk                  = mj * ij_ray_dim + j
        lumMD  (1:nx,1:nnu,jsk,ksk) = recv_buf2(1,1:nx,1:nnu,j,k,m+1)
        e_rmsMD(1:nx,1:nnu,jsk,ksk) = recv_buf2(2,1:nx,1:nnu,j,k,m+1)
      END DO ! j = 1,ij_ray_dim
    END DO ! k = 1,ik_ray_dim
  END DO ! m = 0,n_proc-1
...
!-----------------------------------------------------------------------
!  Compute the internal energy and nuclear binding energy
!-----------------------------------------------------------------------
...
send_buf4(1,:,:,:)         = e_no_bind_in (:,:,:)
send_buf4(2,:,:,:)         = e_bind_in    (:,:,:)
send_buf4(3,:,:,:)         = e_bind_fnl_in(:,:,:)
send_buf4(4,:,:,:)         = a_nuc_rep_in (:,:,:)
DO n = 1,nnc
  send_buf4(4+n,:,:,:)     = xn_in(:,n,:,:)
END DO ! n = 1,nnc
...
c_gath_send                  = nx * (4 + nnc) * ij_ray_dim * ik_ray_dim
c_gath_recv                  = nx * (4 + nnc) * ij_ray_dim * ik_ray_dim

CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
CALL MPI_GATHER( send_buf4, c_gath_send, MPI_DOUBLE_PRECISION, recv_buf4, c_gath_recv, &
& MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr )
...
IF ( myid == 0 ) THEN
  ...
  DO m = 0,n_proc-1
    ...
    DO k = 1,ik_ray_dim
      ...
      DO j = 1,ij_ray_dim
        jsk                  = mj * ij_ray_dim + j
        e_no_bindMD (1:nx,jsk,ksk) = recv_buf4(1,  1:nx,j,k,m+1)
        e_bindMD    (1:nx,jsk,ksk) = recv_buf4(2,  1:nx,j,k,m+1)
        e_bind_fnlMD(1:nx,jsk,ksk) = recv_buf4(3,  1:nx,j,k,m+1)
        a_nuc_repMD (1:nx,jsk,ksk) = recv_buf4(4,  1:nx,j,k,m+1)
        DO n = 1,nnc
          xnMD(1:nx,n,jsk,ksk)     = recv_buf4(4+n,1:nx,j,k,m+1)
        END DO ! n = 1,nnc
      END DO ! j = 1,ij_ray_dim
    END DO ! k = 1,ik_ray_dim
  END DO ! m = 0,n_proc-1
...
&                            = kmev * tMD(imin:imax,jmin:jmax,kmin:kmax)
  DO i = imin,imax
...
!-----------------------------------------------------------------------
!  Minimum, maximum, and mass averaged entropies
!-----------------------------------------------------------------------
