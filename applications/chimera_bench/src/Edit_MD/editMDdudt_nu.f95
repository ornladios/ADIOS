SUBROUTINE editMDdudt_nu( imin, imax, nx, jmin, jmax, ny, kmin, kmax, nz, &
& x_in, y_in, z_in, time, t_tb, dudt_nuMD, nprint )
!-----------------------------------------------------------------------
!
!    File:         editMDdudt_nu
!    Module:       editMDdudt_nu
!    Type:         Subprogram
...
!
!    Purpose:
!  To perform an edit of the neutrino energy deposition rates
!
!    Subprograms call:
...
!  dudt_nucMD   : neutrino energy seposition rate [ergs g^{-1} s^{1}]
...
REAL(KIND=double), INTENT(in), DIMENSION(nx,ny,nz) :: dudt_nuMD      ! energy generation rate by nuclear reactions (ergs g^{-1} s^{1})
...
WRITE (nprint) y_in
WRITE (nprint) z_in
WRITE (nprint) dudt_nuMD

RETURN
END SUBROUTINE editMDdudt_nu
