SUBROUTINE editMDne( imin, imax, nx, jmin, jmax, ny, kmin, kmax, nz, n, &
& nnu, x_in, y_in, z_in, time, t_tb, e_rmsMD, nprint )
!-----------------------------------------------------------------------
!
!    File:         editMDne
!    Module:       editMDne
...
!
!    Purpose:
!  To perform a M-D edit of the neutrino rms energies.
!
!    Subprograms call:
...
!  e_rmsMD      : neutrino rms energies in a zone (MeV)
...
REAL(KIND=double), INTENT(in), DIMENSION(nx,nnu,ny,nz) :: e_rmsMD     ! neutrino rms energy (MeV)
...
WRITE (nprint) y_in
WRITE (nprint) z_in
WRITE (nprint) e_rmsMD(:,n,:,:)

RETURN
END ! SUBROUTINE editMDne
