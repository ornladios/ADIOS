SUBROUTINE nse_test( jr_min, jr_max, rho, t, ye, ij_ray, ik_ray, &
...
!
!    Purpose:
!      To test the first mass zone with nse(j) = 0 to determine whethern
...
USE physcnst_module, ONLY: rmu, cm3fm3, kmev

USE cycle_module, ONLY: ncycle
USE edit_module, ONLY: nlog
USE eos_snc_x_module, ONLY: nse, eosrho, aesv, nuc_number
USE nucbrn_module, ONLY: xn, a_nuc_rep, z_nuc_rep, a_name, inuc
...
INTEGER                          :: ij_radial        ! y-radial zone index
INTEGER                          :: ik_radial        ! z-radial zone index
...
!-----------------------------------------------------------------------
  ...
  101 FORMAT (' Radial zone ',i4,' radial ray',2i4,' has been switched to nse at cycle', &
& i7,' told=',1pe10.3,' tnew=',1pe10.3,' s_nonNSE=',es11.3,' s_NSE=',es11.3)
  151 FORMAT (' inuc=',i4,' I do not know whatto do.')
  201 FORMAT (' Radial zone ',i4,' radial ray',2i4,' has been deflashed at cycle',i7, &
...
!   If inuc = 0 (network off) flashing occurs at a lower temperature
!    as a compromise.
!   If inuc = 1 (network on) flashing occurs at a high temperature to
!    assure a smooth transition to NSE,
!   If inuc = 2 or i_nuc = 4, (network off) set tnse too high for
!    flashing
!
!-----------------------------------------------------------------------

brydns               = rho(j_test) * kfm
IF ( inuc == 0 ) THEN
  tnse               = 0.44d0/kmev
ELSE IF ( inuc == 1 ) THEN
  tnse               = ( rho(j_test) - rho_l ) * slope + t_l
  tnse               = DMIN1( tnse, t_h )
ELSE IF ( inuc == 2  .or.  inuc == 4 ) THEN
  tnse               = 1.d+20
ELSE
  WRITE (nlog,151) inuc
  STOP
...
!-----------------------------------------------------------------------
  ...
  CALL set_cube_j( j_test, rho, t, ye, ij_ray, ik_ray )
...
!-----------------------------------------------------------------------
  ...
  ij_radial          = myid * ij_ray_dim + ij_ray
  ik_radial          = myid * ik_ray_dim + ik_ray
  WRITE (nlog,101) j_test, ij_radial, ik_radial, ncycle, t_nonNSE, &
&  t(j_test), s_nonNSE, s_NSE
  RETURN
...
!  Return if inuc = 3 or inuc = 4
!-----------------------------------------------------------------------

IF ( inuc == 3  .or.  inuc == 4 ) RETURN
j_tstm1              = j_test - 1
IF ( j_tstm1 == 1 ) RETURN
...
!-----------------------------------------------------------------------
  ...
  ij_radial          = myid * ij_ray_dim + ij_ray
  ik_radial          = myid * ik_ray_dim + ik_ray
  WRITE (nlog,201) j_tstm1, ij_radial, ik_radial, ncycle, x_n, x_p, x_He, &
& x_Ni, x_A, a_nuc_rep(j_tstm1), z_nuc_rep(j_tstm1), s_NSE, s_nonNSE
