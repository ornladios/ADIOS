...
REAL(KIND=double)                  :: rmuend         ! nondegenerate approximation for eta * tbck
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (' Convergence failure in electron relativistic Sommerfeld in lectron')
 1003 FORMAT (' jshel=',i3,' d=',es11.3,' tmev=',es11.3,' ye=',es11.3)
 1005 FORMAT (' tol=',es11.3,' xx=',es11.3,' xans**third=',es11.3, &
& ' ffn(xx)=',es11.3,' xans=',es11.3)
 2001 FORMAT (' Convergence failure in electron nonrelativistic Gauss-Laguerre integration in lectron')
 2003 FORMAT (' jshel=',i3,' d=',es11.3,' tmev=',es11.3,' ye=',es11.3)
 2005 FORMAT (' etae=',es11.3,' deta=',es11.3,' tol=',es11.3,' ww*yebc=',es11.3,' wwchk=',es11.3)
 3001 FORMAT (' Convergence failure in electron nonrelativistic Sommerfeld in lectron')
 3003 FORMAT (' jshel=',i3,' d=',es11.3,' tmev=',es11.3,' ye=',es11.3)
...
ef              = efc * ( dbck * yebck )**tthird
nonrel          = .false.
IF ( ef < 0.01 * me  .and.  tbck < 0.01 * me ) nonrel = .true.
...
!-----------------------------------------------------------------------
  ...
  approx        = ( beta <= betmax )
  IF ( approx ) THEN
    etae        = cube( 1.5d0 * ( ww * yebck ), DMAX1( pi2 * third - 0.5d0 * beta2, zero ) )
...
!-----------------------------------------------------------------------
    ...
    IF ( approx ) THEN
      ped       = ( tbck/ww ) * third * ( g3( etae * etae ) - 1.5d0 * beta2 * g1( etae * etae ) )
      pe        = dbck * ped
      se        = ( 4.d0/tbck ) * ped - yebck * etae + beta2 * g1( etae * etae )/ww
      ee        = tbck * ( se + yebck * etae ) - ped
      yeplus    = 2.0d0 * fexp(-etae) * ( 1.0d0 - fexp(-etae)/8.0d0 )/ww

      rel       = 3.0d0 * ped/( ee - me * ( yebck + 2.0d0 * yeplus ) )
      approx = ( rel <= relmin )
...
!-----------------------------------------------------------------------
!  Iterate on etae until wwchk = ww*yebck
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
  ...
  IF ( .not. Failure  .and.  etae - beta < 35.d0 ) THEN
...
!-----------------------------------------------------------------------
    ...
    yeplus      = yeplus/ww
    ue          = etae * tbck
    pe          = ( tbck/ww ) * dbck * third * pe
    ee          = ( tbck/ww ) * ee
    se          = ( ee + pe/dbck )/tbck - yebck * etae
    rel         = 3.0d0 * pe/dbck/( ee - me * ( yebck + 2.0d0 * yeplus ) )
    RETURN
...
!-----------------------------------------------------------------------
  ...
  pfc           = hbarc * ( 3.d0 * pi2 * dbck * yebck )**third
  rb2           = pi2/beta2
  xans          = ( pfc/me )**3
  xx            = pfc/me
  dxxp          = zero
...
!-----------------------------------------------------------------------
  ...
  IF ( Failure ) THEN
    WRITE (nlog,1001)
    WRITE (nlog,1003) jshel,dbck,tbck,yebck
    WRITE (nlog,1005) tol,xx,xans**third,ffn(xx),xans
    WRITE (6,1001)
    WRITE (6,1003) jshel,dbck,tbck,yebck
...
!-----------------------------------------------------------------------
    ...
    rmuend      = tbck * DLOG( dbck * yebck * rmuec/tbck**1.5 )
    etand       = rmuend/tbck
    rintrp      = etad/( 1.d0 + etad )
    etae        = etand + rintrp * ( etad - etand )
    ww          = cnstnr * dbck * yebck/tbck**1.5
...
!-----------------------------------------------------------------------
    ...
    IF ( Failure ) THEN
      WRITE (nlog,2001)
      WRITE (nlog,2003) jshel,dbck,tbck,yebck
      WRITE (nlog,2005) etae,deta,tol,ww*yebck,wwchk
      WRITE (6,2001)
      WRITE (6,2003) jshel,dbck,tbck,yebck
      WRITE (6,2005) etae,deta,tol,ww*yebck,wwchk
    END IF ! Failure
...
!-----------------------------------------------------------------------
      ...
      ek        = ek * tbck * yebck/ww
      ee        = ek + yebck * me
      pe        = 2.d0 * ek * dbck/3.d0
      se        = ( ee + pe/dbck )/tbck - yebck * ue/tbck
      yeplus    = zero
      rel       = zero
      RETURN
...
!-----------------------------------------------------------------------
    ...
    yans        = 3.d0 * pi2 * dbck * yebck * ( hbarc**2/( 2.d0 * me * tbck ) )**1.5
    y           = yans**(tthird)
    dyp         = zero
...
!-----------------------------------------------------------------------
    ...
    IF ( Failure ) THEN
      WRITE (nlog,3001)
      WRITE (nlog,3003) jshel,dbck,tbck,yebck
      WRITE (nlog,3005) tol,y,yans**(tthird)
      WRITE (6,3001)
      WRITE (6,3003) jshel,dbck,tbck,yebck
...
!-----------------------------------------------------------------------
      ...
      pe        = 2.d0 * tbck * pnr(y)/( 15.d0 * pi2 ) * ( 2.d0 * me * tbck/( hbarc**2 ) )**1.5d0
      ek        = 1.5d0 * pe
      ee        = ek/dbck + yebck * me
      ue        = y * tbck + me
      se        = ( ee + pe/dbck - ue * yebck )/tbck
...
REAL (KIND=double) :: xx
dfndx           = 3.d0 * ffn(xx)/xx - pi2/beta2 * ( xx * xx + 1.d0 )/xx**2 - 1.4d0 * pi2/beta2 * pi2/beta2/xx**6
END FUNCTION dfndx
