SUBROUTINE eqstt_x( i, j, ij_ray, ik_ray, rho, t, ye, esv, esvd, esvt, &
& esvy )
!-----------------------------------------------------------------------
!
!    File:         eqstt_x
...
!
!    Purpose:
!      To interpolate the logs of equation of state quantities
...
!
!    Subprograms called:
!  eosidx  : computes the eos table indices
!
!    Input arguments:
...
USE eos_snc_x_module, ONLY : escnst, estble
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (1x)
 1003 FORMAT (1x,20x,'***parameter out of range in eqstt_x***')
 1005 FORMAT (' j=',i4,' ij_ray=',i4,' ik_ray=',i4,' rho=',1pe10.3,' t=',1pe10.3,' ye=',1pe10.3)
...
CALL eosidx_x( j, ij_ray, ik_ray, rho, t, ye, fdm, fdp, fdd, ftm, ftp, &
& ftt, fym, fyp, fyy )
...
IF ( DABS(besv) > alogmx ) THEN
  WRITE (nprint,1001)
  WRITE (nprint,1003)
  WRITE (nprint,1005) j, ij_ray, ik_ray, rho, t, ye
...
esvtmp             = 10.d+00**besv
esv                = esvtmp - escnst(i,j,ij_ray,ik_ray) - epsilon
