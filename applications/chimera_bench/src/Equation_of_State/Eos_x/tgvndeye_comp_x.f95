SUBROUTINE tgvndeye_comp_x( j, ij_ray, ik_ray, rho, e, ye, t_prev, t )
!-----------------------------------------------------------------------
!
!    File:         tgvndeye_comp_x
!    Module:       tgvndeye_comp_x
!    Type:         Subprogram
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (' NR iteration for e will not converge in subroutine tgvndeye_comp_x, &
& will try Bisection')
 1003 FORMAT (' j=',i4,' myid=',i4,' j_ray=',i4,' k_ray=',i4,' nse=',i4, &
& ' rho=',es11.3,' t_prev=',es11.3,' ye=',es11.3,' e=',es20.11,' e_test=',es20.11, &
& ' t_test=',es20.11)
 1005 FORMAT (' j=',i4,' xn=',17es11.3)
 1007 FORMAT (' nes(',i4,')=',i4,' nes(',i4,')=',i4,' nes(',i4,')=',i4)
 2001 FORMAT (' j=',i4,' nse=',i4,' rho=',es15.8,' t_test=', es15.8, &
& ' ye=',es15.8,' e=',es15.8,' e_test=',es15.8)
 2003 FORMAT (' Bisection iteration for e will not converge in subroutine tgvndeye_comp_x')
 2005 FORMAT (' j=',i4,' myid=',i4,' j_ray=',i4,' k_ray=',i4,' nse=',i4, &
& ' rho=',es15.8,' t_prev',es15.8, ' ye=',es15.8,' e=',es20.11,' e_test=',es20.11, &
& ' t_test=',es20.11,' t_min=',es20.11, ' t_max=',es20.11)
...
DO it = 1,itmax
  ...
  CALL eqstt_x( ivar, j, ij_ray, ik_ray, rho, t_test, ye, e_test, dedd, dedt, dedy )
...
WRITE (nprint,1001)
WRITE (nprint,1003) j, myid, j_ray, k_ray, nse(j,ij_ray,ik_ray), rho, t_prev, &
& ye, e, e_test, t_test
WRITE (nprint,1005) (jj,(xn(jj,nc),nc=1,nnc),jj=j-1,j+1)
WRITE (nprint,1007) j-1, nse(j-1,ij_ray,ik_ray), j, nse(j,ij_ray,ik_ray), &
& j+1, nse(j+1,ij_ray,ik_ray)
WRITE (nlog,1001)
WRITE (nlog,1003) j, myid, j_ray, k_ray, nse(j,ij_ray,ik_ray), rho, t_prev,   &
& ye, e, e_test, t_test
WRITE (nlog,1005) (jj,(xn(jj,nc),nc=1,nnc),jj=j-1,j+1)
WRITE (nlog,1007) j-1, nse(j-1,ij_ray,ik_ray), j ,nse(j,ij_ray,ik_ray),   &
& j+1, nse(j+1,ij_ray,ik_ray)
...
&  dedt, dedy )
  ...
  WRITE (nlog,2001) j, nse(j,ij_ray,ik_ray), rho, t_test, ye, e, e_test
...
WRITE (nprint,2003)
WRITE (nprint,2005) j, myid, j_ray, k_ray, nse(j,ij_ray,ik_ray), rho, &
& t_prev, ye, e, e_test, t_test, t_min, t_max
WRITE (nprint,1005) (jj,(xn(jj,nc),nc=1,nnc),jj=j-1,j+1)
WRITE (nlog,2003)
WRITE (nlog,2005) j, myid, j_ray, k_ray, nse(j,ij_ray,ik_ray), rho,   &
& t_prev, ye, e, e_test, t_test, t_min, t_max
WRITE (nlog,1005) (jj,(xn(jj,nc),nc=1,nnc),jj=j-1,j+1)
STOP
...
RETURN
END SUBROUTINE tgvndeye_comp_x
