...
!-----------------------------------------------------------------------
!  nse(j,ji_ray,jk_ray) : a nuclear statistical equilibrium flag for
!   radial zone j.
!
!     nse(j,ji_ray,jk_ray) = 0 : material not in nuclear statistical
!      equilibrium; nuclear reaction network must be turned on to evolve
!      the matter composition.
!     nse(j,ji_ray,jk_ray) = 1 : material in nuclear statistical
...
!-----------------------------------------------------------------------
!  aesv(j,i,ji_ray,jk_ray)  : equation of state dependent variable i at
!   radial zone j.
!  aesvd(j,i,ji_ray,jk_ray) : derivative with respect to the density of
!   equation of state dependent variable i at radial zone j.
!  aesvt(j,i,ji_ray,jk_ray) : derivative with respect to the temperature
!   of equation of state dependent variable i at radial zone j.
!  aesvy(j,i,ji_ray,jk_ray) : derivative with respect to the electron
...
!     i = 12  : gamma1
!
!  gam1(j,ji_ray,jk_ray)    : first adiabatic index for zone j.
!  gam2(j,ji_ray,jk_ray)    : second adiabatic index for zone j.
!  gam3(j,ji_ray,jk_ray)    : third adiabatic index for zone j.
!
!  duesrc(j,ji_ray,jk_ray)  : cumulative energy 'glitches' per unit mass
...
!
!  idty(j,ji_ray,jk_ray)  : the density regime (i.e., 1, 2, or 3) of radial
!   zone j as given by the above inequalities.
!-----------------------------------------------------------------------
...
!  Cube indices
!-----------------------------------------------------------------------
!  idr(j,ji_ray,jk_ray), itr(j,ji_ray,jk_ray), and iyr(j,ji_ray,jk_ray) :
!   integers defining the location of log(rho), log(t), and ye for radial
!   zone j on the grid, or n-neutrino bremsstrahlung, i.e.,
!
!     idr(j,ji_ray,jk_ray)/dgrid < log(rho(j)) < ( idr(j,ji_ray,jk_ray) + 1 )/dgrid 
!
!     itrb(j,ji_ray,jk_ray)/tgrid <  log(t(j))  < ( itrb(j,ji_ray,jk_ray) + 1 )/tgrid
!
!     0.5 - iyrb(j,ji_ray,jk_ray)/ygrid < ye < 0.5 - ( iyrb(j,ji_ray,jk_ray) + 1 )/ygrid
!
...
!  EOS table
!-----------------------------------------------------------------------
!  estble(i,j,id,it,iy,ji_ray,jk_ray) : equation of state variable i of
...
!
!  escnst(i,j,ji_ray,jk_ray) : constant that is added to equation of state
...
!
!  be_nuc_rep(j) : binding energy of the representative heavy nucleus
!   (MeV).
!
