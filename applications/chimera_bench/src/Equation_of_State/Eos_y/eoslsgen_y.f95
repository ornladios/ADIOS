SUBROUTINE eosdtgen_y( j, ji_ray, jk_ray, idd, itt, iyy, ida, ita, iya, sf )
!-----------------------------------------------------------------------
!
!    File:         eoslsgen_y
!    Module:       eosdtgen_y
...
!       of state (or subroutine eos_y, the Cooperstein-BCK equation
...
!
!  nse(j,ji_ray,jk_ray) : nuclear statistical equilibrium flag for radial zone j.
...
!  eos_y                : invokes the BCK eos
!
!    Include files:
!  kind_module, numerical_module, physcnst_module
!  edit_module, el_eos_module, eos_bck_module, eos_bck_module,
!  eos_ls_module, eos_m4c_module, eos_snc_y_module
!
!-----------------------------------------------------------------------
...
USE eos_snc_y_module, ONLY: eos_i, idty, dgrid, tgrid, ygrid, eosrho, nse, &
& estble
...
INTEGER, INTENT(out)             :: sf            ! eos_y flag
...
INTEGER                          :: ii            ! eos_y dependent variable index
...
REAL(KIND=double), DIMENSION(12,2,2,2) :: temp_store ! temporary storage for eos_y table entries

  101 FORMAT ('Equation of state identifier does not match either L or B')
 1001 FORMAT (' sf = 0 in eosdtgen_y; j=',i3,' ji_ray=',i3,' jk_ray=',i3, &
...
rhod               = 10.d0**( DBLE(idd)/dgrid(idty(j,ji_ray,jk_ray)) )
td                 = 10.d0**( DBLE(itt)/tgrid(idty(j,ji_ray,jk_ray)) )
yed                = one  - ( DBLE(iyy)/ygrid(idty(j,ji_ray,jk_ray)) )
yed                = DMIN1( yed, one )
...
&      eos_i /= 'S' )                             THEN
  ...
  ELSE
    eos_ii         = 'B'
  END IF ! brydns > eosrho, tmev > eost, nse(j,ji_ray,jk_ray) = 1, eos_i /= 'S'
...
!-----------------------------------------------------------------------
  ...
  DO id = 1,2
    DO it = 1,2
      DO iy = 1,2
        inpvars(j,1,id,it,iy) = ( one - ( DBLE(iyy+iy-1)/ygrid(idty(j,ji_ray,jk_ray)) ) ) * brydns
...
!-----------------------------------------------------------------------
    ...
    IF ( sf == 0 ) then
      WRITE (nprint,1001) j, ji_ray, jk_ray, rhod, td, yed
      RETURN
    END IF ! sf = 0
  ...
  IF ( ida == 2  .and.  ita == 2  .and.  iya == 2 ) THEN
    DO iy = 1,2
      DO it = 1,2
        DO id = 1,2
          DO ii = 1,12
            estble(ii,j,id,it,iy,ji_ray,jk_ray) = temp_store(ii,id,it,iy)
          END DO
        END DO
      END DO
    END DO
...
!-----------------------------------------------------------------------
  ...
  estble(1 ,j,ida,ita,iya,ji_ray,jk_ray) = ps
  estble(2 ,j,ida,ita,iya,ji_ray,jk_ray) = us
  estble(3 ,j,ida,ita,iya,ji_ray,jk_ray) = ss
  estble(4 ,j,ida,ita,iya,ji_ray,jk_ray) = cmpns
  estble(5 ,j,ida,ita,iya,ji_ray,jk_ray) = cmpps
  estble(6 ,j,ida,ita,iya,ji_ray,jk_ray) = ue
  estble(7 ,j,ida,ita,iya,ji_ray,jk_ray) = xnbck
  estble(8 ,j,ida,ita,iya,ji_ray,jk_ray) = xpbck
  estble(9 ,j,ida,ita,iya,ji_ray,jk_ray) = xhbck
  estble(10,j,ida,ita,iya,ji_ray,jk_ray) = ahbck
  estble(11,j,ida,ita,iya,ji_ray,jk_ray) = zs
...
!-----------------------------------------------------------------------
  ...
  estble(12,j,ida,ita,iya,ji_ray,jk_ray) = DMAX1( gamma1, tiny )
