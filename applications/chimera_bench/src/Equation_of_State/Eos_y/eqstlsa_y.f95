SUBROUTINE eqstta_y( i, j, ji_ray, jk_ray, rhoj, tj, yej, v, vd, vt, vy )
!-----------------------------------------------------------------------
!
!    File:         eqstlsa_y
!    Module:       eqstta_y
...
!  eos_bck_module, eos_snc_y_module
!
!-----------------------------------------------------------------------
...
USE eos_snc_y_module, ONLY: eos_i, nse, eosrho
...
INTEGER, INTENT(in)              :: i             ! eos_y dependent variable index
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (' sf = 0 in eqstta_y; j=',i3,' ji_ray=',i3,' jk_ray=',i3, &
...
!               i            ne 12
!        then            use Lattimer-Swesty eos_y
!        otherwise,      use Cooperstein bck eos_y
!-----------------------------------------------------------------------
...
ELSE
  eos_ii           = 'B'
END IF ! brydns >= eosrho, tmev >= eost, nse(j,ji_ray,jk_ray) == 1, i /= 12, eos_i /= 'S'
...
!-----------------------------------------------------------------------
  ...
  IF (sf == 0 ) WRITE (nprint,1001) j, ji_ray, jk_ray, rhoj, tj, ye_inveos
...
END IF ! brydns > eosrho, tmev > eost, nse(j,ji_ray,jk_ray) = 1, eos_i = 'L'
