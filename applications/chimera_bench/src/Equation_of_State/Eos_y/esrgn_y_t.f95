SUBROUTINE esrgn_y_t( j, ji_ray, jk_ray, rho, t, ye )
!-----------------------------------------------------------------------
!
!    File:         esrgn_y_t
!    Module:       esrgn_y_t
...
REAL(KIND=double), DIMENSION(2,2) :: estmx
  ...
  101 FORMAT (' Table generation unsuccessful in esrgn_y_t, ncycle=',i7, &
...
IF ( it == itr(j,ji_ray,jk_ray) ) RETURN
...
DO id = 1,2
  DO it = 1,2
    DO iy = 1,2
      DO i = 1,12
        estble(i,j,id,it,iy,ji_ray,jk_ray) = DLOG10( estble(i,j,id,it,iy,ji_ray,jk_ray) + escnst(i,j,ji_ray,jk_ray) + epsilon )
      END DO
    END DO
  END DO
END DO
...
DO id = 1,2
  DO iy = 1,2
    estmn(id,iy)   = AMIN1( estble(2,j,id,1,iy,ji_ray,jk_ray), estble(2,j,id,2,iy,ji_ray,jk_ray) )
    estmx(id,iy)   = AMAX1( estble(2,j,id,1,iy,ji_ray,jk_ray), estble(2,j,id,2,iy,ji_ray,jk_ray) )
