SUBROUTINE tgvndsye_comp_y( j, ji_ray, jk_ray, rho, s, ye, t_prev, t )
!-----------------------------------------------------------------------
!
!    File:         tgvndsye_comp_y
!    Module:       tgvndsye_comp_y
!    Type:         Subprogram
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (' NR iteration for e will not converge in subroutine tgvndsye_comp_y, &
 &will try Bisection')
...
DO it = 1,itmax
  ...
  CALL eqstt_y( ivar, j, ji_ray,jk_ray, rho, t_test, ye, s_test, dsdd, dsdt, dsdy )
...
& nse(j,ji_ray,jk_ray), rho, t_prev, ye, s, s_test, t_test
...
&  dsdt, dsdy )
  ...
  WRITE (nlog,2001) it, nse(j,ji_ray,jk_ray), rho, t_test, ye, s, s_test, &
&  tmin, tmax
...
WRITE (nprint,1005) (jj,(xn(jj,nc),nc=1,nnc),jj=j-1,j+1)
WRITE (nlog,2003)
WRITE (nlog,2005) j, j_ray_bndl, k_ray_bndl, j_ray_bndl, k_ray_bndl, j_angular, &
& k_angular, myid, nse(j,ji_ray, jk_ray), rho, t_prev, ye, s, s_test, t_test
WRITE (nlog,1005) (jj,(xn(jj,nc),nc=1,nnc),jj=j-1,j+1)
STOP
...
RETURN
END SUBROUTINE tgvndsye_comp_y
