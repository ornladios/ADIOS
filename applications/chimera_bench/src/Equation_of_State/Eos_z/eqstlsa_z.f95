SUBROUTINE eqstta_z( i, k, ki_ray, kj_ray, rhoj, tj, yej, v, vd, vt, vy )
!-----------------------------------------------------------------------
!
!    File:         eqstlsa_y
!    Module:       eqstta_z
...
!   i = 12      : gamma1.
!   i = 13      : equation of state quantities are obtained from the Cooperstein-BCK equation of state.                  !
!
...
!  eos_bck_module, eos_snc_z_module
!
!-----------------------------------------------------------------------
...
USE eos_snc_z_module, ONLY: eos_i, nse, eosrho
...
INTEGER, INTENT(in)              :: i             ! eos_z dependent variable index
...
INTEGER, PARAMETER               :: nyd = 500

CHARACTER (LEN=1)                :: eos_ii

LOGICAL                          :: first = .true.
LOGICAL                          :: firstbck = .true.
LOGICAL, DIMENSION(nyd)          :: firstls

INTEGER                          :: ii            ! do index
INTEGER                          :: jj            ! radial zone do index
INTEGER                          :: sf            ! eos_z flag
...
!-----------------------------------------------------------------------
  ...
  201 FORMAT (' ny=',i3,' exceeds nyd=',i3,' in subroutine sctekrnl')
 1001 FORMAT (' sf = 0 in eqstta_z; k=',i3,' ki_ray=',i3,' kj_ray=',i3, &
...
IF ( ny > nyd ) THEN
  WRITE (nprint,201) ny,nyd
  WRITE (nlog,201) ny,nyd
  STOP
END IF
...
!               i            ne 12
!        then            use Lattimer-Swesty eos_z
!        otherwise,      use Cooperstein bck eos_z
!-----------------------------------------------------------------------

IF ( brydns >= eosrho           .and.         &
&    tmev >= eost               .and.         &
&    nse(k,kj_ray,ki_ray) == 1  .and.         &
& i /= 12  .and.  eos_i /= 'S' )                THEN
  eos_ii           = 'L'
ELSE
  eos_ii           = 'B'
END IF ! brydns >= eosrho, tmev >= eost, nse(k,kj_ray,ki_ray) == 1, i /= 12, eos_i /= 'S'
...
!-----------------------------------------------------------------------
  ...
  IF ( firstls(k) ) THEN
    inpvarsa(k,1)  = yej * brydns
    inpvarsa(k,2)  = 0.155d+00
    inpvarsa(k,3)  = -15.d+00
    inpvarsa(k,4)  = -10.d+00
    firstls(k)     = .false.
  END IF ! firstls(k)
...
!-----------------------------------------------------------------------
  ...
  pprev            = DMIN1( inpvarsa(k,1), brydns * yej )
  inpvar(1)        = tmev
  inpvar(2)        = inpvarsa(k,2)
  inpvar(3)        = inpvarsa(k,3)
  inpvar(4)        = inpvarsa(k,4)

  ye_inveos        = DMIN1( yej, 0.5d0 )

  CALL inveos( inpvar, t_old, ye_inveos, brydns, iflag, eosflg, forflg, sf, xprev, pprev )
  IF (sf == 0 ) WRITE (nprint,1001) k, ki_ray, kj_ray, rhoj, tj, ye_inveos
...
!-----------------------------------------------------------------------
  ...
  inpvarsa(k,1)    = pprev
  inpvarsa(k,2)    = inpvar(2)
  inpvarsa(k,3)    = inpvar(3)
  inpvarsa(k,4)    = inpvar(4)
...
!-----------------------------------------------------------------------
!        Load v, vd, vt, and vr, changing from MeV and fm to
!         cgs units, as appropriate.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!  Initialize BCK inputs for a particular zone
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!  Convert thermodynamic quantities from MeV and fm to cgs units,
!   where appropriate.
!-----------------------------------------------------------------------
...
END IF ! brydns > eosrho, tmev > eost, nse(k,kj_ray,ki_ray) = 1, eos_i = 'L'
