SUBROUTINE eqstt_z( i, k, ki_ray, kj_ray, rho, t, ye, esv, esvd, esvt, &
& esvy )
!-----------------------------------------------------------------------
!
!    File:         eqstt_z
...
!
!    Subprograms called:
!  eosidx_z  : computes EOS table indices and interpolation coefficients
!
!    Input arguments:
...
USE eos_snc_z_module, ONLY : escnst, estble
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (1x)
 1003 FORMAT (1x,20x,'***parameter out of range in eqstt_z***')
 1005 FORMAT (' k=',i4,' ki_ray=',i4,' kj_ray=',i4,' rho=',1pe10.3,' t=',1pe10.3, &
& ' ye=',1pe10.3)
...
CALL eosidx_z( k, ki_ray, kj_ray, rho, t, ye, fdm, fdp, fdd, ftm, ftp, &
& ftt, fym, fyp, fyy )
...
IF ( DABS(besv) > alogmx ) THEN
  WRITE (nprint,1001)
  WRITE (nprint,1003)
  WRITE (nprint,1005) k, ki_ray, kj_ray, rho, t, ye
  ...
  WRITE (nlog,1001)
  WRITE (nlog,1003)
  WRITE (nlog,1005) k, ki_ray, kj_ray, rho, t, ye
...
esvtmp             = 10.d+00**besv
esv                = esvtmp - escnst(i,k,kj_ray,ki_ray) - epsilon
