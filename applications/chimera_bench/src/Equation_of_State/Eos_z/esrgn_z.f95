SUBROUTINE esrgn_z( k, ki_ray, kj_ray, rho, t, ye )
!-----------------------------------------------------------------------
!
!    File:         esrgn_z
!    Module:       esrgn_z
...
!       created. Subroutine eosdtgen_z is called to fill this table
!       with eos quantities.
!
!    Input arguments:
!
...
REAL(KIND=double), DIMENSION(2,2) :: estmx
  ...
  101 FORMAT (' Table generation unsuccessful in esrgn_z, ncycle=',i7, &
& ' k=',i4,' ki_ray=',i4,'ki_ray=',i4,' rho, t, ye=',3(1pe11.3))
...
ye_min                            = 2.d0/ygrid(idty(k,kj_ray,ki_ray))
yed                               = DMAX1( ye, ye_min )
...
fy                                = ygrid(idty(k,kj_ray,ki_ray)) * (one - yed)
...
IF ( id == idr(k,kj_ray,ki_ray)  .and.      &
&    it == itr(k,kj_ray,ki_ray)  .and.      &
&    iy == iyr(k,kj_ray,ki_ray) )             RETURN
...
DO id = 1,2
  DO it = 1,2
    DO iy = 1,2
      DO i = 1,12
        estble(i,k,id,it,iy,kj_ray,ki_ray)                              &
&                                 = DLOG10( estble(i,k,id,it,iy,kj_ray,ki_ray) &
