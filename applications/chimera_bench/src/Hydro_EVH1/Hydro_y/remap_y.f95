SUBROUTINE remap_y( ngeom, jmin, jmax, ji_ray, jk_ray, ny, nez, nnu, &
& nnc, j_shock )
!-----------------------------------------------------------------------
!
!    File:         remap_y
...
USE eos_snc_y_module, ONLY: nse, xn, nuc_number, a_nuc, z_nuc, a_nuc_rep, z_nuc_rep
USE evh1_global, ONLY: smallr, nlefty, nrighty, v_diff
...
INTEGER, INTENT(in), DIMENSION(ny)           :: j_shock    ! zones marked for added y-diffusion
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (' Allocation problem for array ',a10,' in remap_y')
 2001 FORMAT (' Deallocation problem for array ',a10,' in remap_y')
...
CALL parabola( nmin-3, nmax+3, ntot, zparay, r,    dr,  r6,  rl,  dm, 0, &
& 0, ngeom )
CALL parabola( nmin-3, nmax+3, ntot, zparay, u,    du,  u6,  ul,  dm, 0, &
& 0, ngeom )
CALL parabola( nmin-3, nmax+3, ntot, zparay, v,    dv,  v6,  vl,  dm, 0, &
& 0, ngeom )
CALL parabola( nmin-3, nmax+3, ntot, zparay, w,    dw,  w6,  wl,  dm, 0, &
& 0, ngeom )
CALL parabola( nmin-3, nmax+3, ntot, zparay, ei_b, dei, ei6, eil, dm, 0, &
& 0, ngeom )
CALL parabola( nmin-3, nmax+3, ntot, zparay, ye,   dye, ye6, yel, dm, 0, &
& 0, ngeom )
CALL parabola( nmin-3, nmax+3, ntot, zparay, se,   ds,  s6,  sl,  dm, 0, &
& 0, ngeom )
...
CALL parabola( nmin-3, nmax+3, ntot, zparay, e, de, e6, el, dm, 0, 1,   &
& ngeom )
...
!-----------------------------------------------------------------------
    ...
    IF ( nse(j,ji_ray,jk_ray) == 1 ) fluxbe(n) = eb(n) * fluxr(n)
    IF ( nse(j,ji_ray,jk_ray) == 0 ) fluxye(n) = fluxye_comp(n)
...
CALL paraset( ntot, zparay, dx0, xa0, nmin-3, nmax+3, ngeom )
