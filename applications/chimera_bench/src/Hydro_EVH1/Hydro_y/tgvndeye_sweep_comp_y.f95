SUBROUTINE tgvndeye_sweep_comp_y( ni, nf, ji_ray, jk_ray, rho_f, rho_i )
!-----------------------------------------------------------------------
!
!    File:         tgvndeye_sweep_comp_y
!    Module:       tgvndeye_sweep_comp_y
!    Type:         Subprogram
...
!  tgvndsye_comp_y : updates the temperature if the degeneracy parameter is high
!  tgvndeye_comp_y : updates the temperature if the degeneracy parameter is low
!  eqstt_y         : interpolates eos quantities
!
!    Input arguments:
...
&  ' k_angular',i4,' proc', i4,' in tgvndeye_sweep_comp_y; re(n)=', &
...
!-----------------------------------------------------------------------
    ...
    CALL tgvndsye_comp_y( n-6, ji_ray, jk_ray, re(n), se(n), yee(n), &
&    temp(n), t_out )
    temp(n)         = t_out
...
!-----------------------------------------------------------------------
    ...
    CALL tgvndeye_comp_y( n-6, ji_ray, jk_ray, re(n), ei(n), yee(n), &
&    temp(n), t_out )
    temp(n)         = t_out
...
RETURN
END SUBROUTINE tgvndeye_sweep_comp_y
