SUBROUTINE tgvndeye_sweep_z( ni, nf, ki_ray, kj_ray, rho_f, rho_i )
!-----------------------------------------------------------------------
!                                                                      !
!    File:         tgvndeye_sweep_z
!    Module:       tgvndeye_sweep_z
!    Type:         Subprogram
...
!  tgvndsye_z : unpdates the temperature by iterating on the entropy
!  tgvndeye_z : unpdates the temperature by iterating on the energy
!  eqstt_z    : interpolates EOS quantities from cube corners to a given state
!
!    Input arguments:
...
&  ' k_angular',i4,' proc', i4,' in tgvndeye_sweep_z; re(n)=',es11.3, &
...
!-----------------------------------------------------------------------
    ...
    CALL tgvndsye_z( n-6, ki_ray, kj_ray, re(n), se(n), yee(n), temp(n), &
&    t_out )
    temp(n)         = t_out
...
!-----------------------------------------------------------------------
    ...
    CALL tgvndeye_z( n-6, ki_ray, kj_ray, re(n), ei(n), yee(n), temp(n), &
&    t_out )
    temp(n)         = t_out
...
RETURN
END SUBROUTINE tgvndeye_sweep_z
