...
!
!     idrae(j,ij_ray,ik_ray)/dgrid < log(rho(j)) < ( idrae(j,ij_ray,ik_ray) + 1 )/dgrid 
!
!     itrae(j,ij_ray,ik_ray)/tgrid <  log(t(j))  < ( itrae(j,ij_ray,ik_ray) + 1 )/tgrid
!
!     0.5 - iyrae(j,ij_ray,ik_ray)/ygrid < ye < 0.5 - ( iyrae(j,ij_ray,ik_ray) + 1 )/ygrid
!
...
!-----------------------------------------------------------------------
!  rncem(k,kp,ij_ray,ik_ray) stores the e-neutrino-nucleus absorption rate
...
!   ab and em below.
!
!  rncep(k,kp,ij_ray,ik_ray) stores the e-antineutrino-nucleus absorption
!   rate (per iron-like nucleus) and per incident neutrino of energy k
!   and final electrons of energy kp as given by Haxton.
!   Absorption and emission rates are computed from this array in
!   subroutine abemhnc, and the results contribute to the arrays
!   ab and em below.
...
!-----------------------------------------------------------------------
!  em(j,k,n,id,it,iy,f(ij_ray,ik_ray)) is an array of emission rates for
!   neutrinos as a function of neutrino type (n), energy (k) and radial
!   zone (j) at the unit cube corners id, it, and iy (id, it, iy = 1,2).
!   Interpolations of emission rates are computed from this table.
!
!  ab(j,k,n,id,it,iy,f(ij_ray,ik_ray)) is an array of absorption rates for
!   neutrinos as a function of neutrino type (n), energy (k) and radial
!   zone (j) at the unit cube corners id, it, and iy (id, it, iy = 1,2).
...
!-----------------------------------------------------------------------
!  emis(j,k,n,ij_ray,ik_ray) : is the emission inverse mean free path
!   (1/cm) for radial zone j, energy zone k, neutrino type n
!
...
!-----------------------------------------------------------------------
!
!  absor(j,k,n,ij_ray,ik_ray) : is the absorption inverse mean free path
!   (1/cm) for radial zone j, energy zone k, neutrino type n
!
