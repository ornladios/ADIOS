SUBROUTINE abemrate( jr_min, jr_max, ij_ray, ik_ray, rho_in, t_in, ye_in, &
& nx )
!-----------------------------------------------------------------------
!
!    File:         abemrate
...
!
!    Purpose:
!      To interpolate, for all energy zones k, all radial zones j,
...
!  idrae(j,ij_ray,ik_ray)     : rho grid index for zone j
!  itrae(j,ij_ray,ik_ray)     : t grid index for zone j
!  iyrae(j,ij_ray,ik_ray)     : ye grid index for zone j
!
!    Output arguments (common):
!
!  emis(j,k,n,ij_ray,ik_ray)  : emission inverse mean free path (1/cm) for radial zone j, energy zone k,
!                        neutrino type n
!  emisd(j,k,n)       : d(emis(j,k,n))/d(rho(j))
!  emist(j,k,n)       : d(emis(j,k,n))/d(t  (j))
!  emisy(j,k,n)       : d(emis(j,k,n))/d(ye (j))
!  absor(j,k,n,ij_ray,ik_ray) : absorption inverse mean free path (1/cm) for radial zone j, energy zone k,
!                        neutrino type n
...
USE abem_module, ONLY : em, ab, emis, emisd, emist, emisy, absor, absord, &
& absort, absory, idrae, itrae, iyrae
...
REAL(KIND=double), INTENT(in), DIMENSION(nx)    :: rho_in ! density (g cm^{-3})
REAL(KIND=double), INTENT(in), DIMENSION(nx)    :: t_in   ! temperature (K)
REAL(KIND=double), INTENT(in), DIMENSION(nx)    :: ye_in  ! electron fraction
...
IF ( nnugpmx == 0  .or.  ( iaefnp == 0  .and.  iaencnu == 0  .and.  iaence == 0 ) ) THEN
  ...
  emis (jr_min:jr_max,:,:,ij_ray,ik_ray)  = zero
  emisd(jr_min:jr_max,:,:)                = zero
  emist(jr_min:jr_max,:,:)                = zero
  emisy(jr_min:jr_max,:,:)                = zero
  absor (jr_min:jr_max,:,:,ij_ray,ik_ray) = zero
  absord(jr_min:jr_max,:,:)               = zero
  absort(jr_min:jr_max,:,:)               = zero
  absory(jr_min:jr_max,:,:)               = zero

  RETURN
END IF ! nnugpmx == 0  .or.  ( iaefnp == 0  .and.  iaencnu == 0  .and.  iaence == 0
...
!-----------------------------------------------------------------------
  ...
  id               = idrae(j,ij_ray,ik_ray)
  it               = itrae(j,ij_ray,ik_ray)
  iy               = iyrae(j,ij_ray,ik_ray)

  rhomin           = 10.d+00**( DBLE(id  )/dgrid(idty(j,ij_ray,ik_ray)) )
  rhomax           = 10.d+00**( DBLE(id+1)/dgrid(idty(j,ij_ray,ik_ray)) )
  tmin             = 10.d+00**( DBLE(it  )/tgrid(idty(j,ij_ray,ik_ray)) )
  tmax             = 10.d+00**( DBLE(it+1)/tgrid(idty(j,ij_ray,ik_ray)) )
  yemax            = one     -( DBLE(iy  )/ygrid(idty(j,ij_ray,ik_ray)) )
  yemin            = one     -( DBLE(iy+1)/ygrid(idty(j,ij_ray,ik_ray)) )
...
!-----------------------------------------------------------------------
  ...
  fd               = dgrid(idty(j,ij_ray,ik_ray)) * LOG10( rho  )
  id               = idrae(j,ij_ray,ik_ray)
  fdp              = fd - DBLE( id )
  fdm              = one - fdp
  fdd              = log_e * dgrid(idty(j,ij_ray,ik_ray))/( rho  )

  ft               = tgrid(idty(j,ij_ray,ik_ray)) * LOG10( t    )
  it               = itrae(j,ij_ray,ik_ray)
  ftp              = ft - DBLE( it )
  ftm              = one - ftp
...
&                  +          fdp * ( ftm * em212 + ftp * em222 ) )
      ...
      emis(j,k,n,ij_ray,ik_ray)  = f10(emisl)
...
&                  +                 ftp * ( -em122 + em222 ) ) )
      ...
      emisd(j,k,n) = ln_10 * emisdl * emis(j,k,n,ij_ray,ik_ray)
...
&                  +                 fdp * ( -em212 + em222 ) ) )
      ...
      emist(j,k,n) = ln_10 * emistl * emis(j,k,n,ij_ray,ik_ray)
...
&                  +                 ftp * ( -em221 + em222 ) ) )
      ...
      emisy(j,k,n) = ln_10 * emisyl * emis(j,k,n,ij_ray,ik_ray)
...
&                  +          fdp * ( ftm * ab212 + ftp * ab222 ) )
      ...
      absor(j,k,n,ij_ray,ik_ray) = f10(absorl)
...
&                  +                 ftp * ( -ab122 + ab222 ) ) )
      ...
      absord(j,k,n)= ln_10 * absordl * absor(j,k,n,ij_ray,ik_ray)
...
&                  +                 fdp * ( -ab212 + ab222 ) ) )
      ...
      absort(j,k,n)= ln_10 * absortl * absor(j,k,n,ij_ray,ik_ray)
...
&                  +                 ftp * ( -ab221 + ab222 ) ) )
      ...
      absory(j,k,n)= ln_10 * absoryl * absor(j,k,n,ij_ray,ik_ray)


!-----------------------------------------------------------------------
!                  ||||| End loop over k ad n |||||
!-----------------------------------------------------------------------
