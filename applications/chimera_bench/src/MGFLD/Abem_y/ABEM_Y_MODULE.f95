...
!
!     idrae(j,ji_ray,ik_ray)/dgrid < log(rho(j)) < ( idrae(j,ji_ray,ik_ray) + 1 )/dgrid 
!
!     itrae(j,ji_ray,ik_ray)/tgrid <  log(t(j))  < ( itrae(j,ji_ray,ik_ray) + 1 )/tgrid
!
!     0.5 - iyrae(j,ji_ray,ik_ray)/ygrid < ye < 0.5 - ( iyrae(j,ji_ray,ik_ray) + 1 )/ygrid
!
...
!-----------------------------------------------------------------------
!  rncem(k,kp,ji_ray,ik_ray) stores the e-neutrino-nucleus absorption rate
...
!   ab and em below.
!
!  rncep(k,kp,ji_ray,ik_ray) stores the e-antineutrino-nucleus absorption rate
!   (per iron-like nucleus) and per incident neutrino of energy k
!   and final electrons of energy kp as given by Haxton.
!   Absorption and emission rates are computed from this array in
!   subroutine abemhnc, and the results contribute to the arrays
...
!-----------------------------------------------------------------------
!  em(j,k,n,id,it,iy,ji_ray,ik_ray) is an array of emission rates for
...
!
!  ab(j,k,n,id,it,iy,ji_ray,ik_ray) is an array of absorption rates for
!   neutrinos as a function of neutrino type (n), energy (k) and angular
!   zone (j) at the unit cube corners id, it, and iy (id, it, iy = 1,2).
