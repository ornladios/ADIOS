SUBROUTINE abem_cal_y( n, e_in, rho, t, xneut, xprot, xh, ah, zh, cmpn, cmpp, &
...
REAL(KIND=double)                :: etap          ! - ( e_in + cmpp + cmpe - dmnp - cmpn )/tmev 
...
IF ( i_aeps == 0  .or.  rho < 1.d+07 ) THEN
  IF ( i_abemetanp ) THEN
...
&    cmp_trgt_i, cmp_trgt_f, cmp_lep, ab_r0_nub, ab_r1_nub, e_out_p )
    absornp        = ab_r0_nub
    etap           = - ( e_in + cmpp + cmpe - dmnp - cmpn )/tmev
    emitnp         = fexp(etap) * absornp
