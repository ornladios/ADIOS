SUBROUTINE paired( n, j, ij_ray, ik_ray, k, r, rho, t, ye, a0w, b0w, &
...
!
!      The zero moment of the production rate per state, dpsi0dt, of neutrinos of
!       type n is given by
!
...
!-----------------------------------------------------------------------
!  No pair annihilation and production if ipair = 0
!-----------------------------------------------------------------------

pair_off             = .false.
IF ( ipair == 0 ) pair_off = .true.
...
CALL pairkrnl( n, j, ij_ray, ik_ray, k, rho, t, ye, f0a, f1a, f0ad, f1ad, &
...
!-----------------------------------------------------------------------
  ...
  art              = art   + f0p * ( one - x0a )           * w2dw(kp)
  brt              = brt   + f1p * x1a                     * w2dw(kp)
