SUBROUTINE pairrgn( j, ij_ray, ik_ray, idd, itt, iyy, ida, ita, iya )
!-----------------------------------------------------------------------
!
!    File:         pairrg
!    Module:       pairrgn
...
!-----------------------------------------------------------------------
!
!        The zero and first legendre coefs for the n-type neutrino pair annihilation
...
!            K    = 2*pi/! * 1/(hc)**3
!
!        The Legendre moments of the neutrino pair annihilation and production functions
!         are given by
!
...
!
!  rhopairemn           : density below which n_e - n_ebar pair annihilation
!                          is not computed (pair annihilation functions are zeroed).
!  rhopairemn           : density above which n_e - n_ebar pair annihilation
!                          is not computed (pair annihilation functions are zeroed).
!  rhopairxmn           : density below which n_x - n_xbar pair annihilation
!                          is not computed (pair annihilation functions are zeroed).
!  rhopairxmn           : density above which n_x - n_xbar pair annihilation 
!                          is not computed (pair annihilation functions are zeroed).
...
USE pair_module, ONLY : paira0i, paira0ii
...
REAL(KIND=double)                :: j0i           ! zero moment of pair annihilation function type i
REAL(KIND=double)                :: j0ii          ! zero moment of pair annihilation function type ii
REAL(KIND=double)                :: j1i           ! zero moment of pair annihilation function type i
REAL(KIND=double)                :: j1ii          ! zero moment of pair annihilation function type ii
...
!   neutrino-antineutrino pair annihilation functions.
!-----------------------------------------------------------------------
...
DO k = 1,nnugpmx
  ...
  DO kp = 1,nnugpmx
    enubar         = unu(j,kp)
    CALL paircal( enu, enubar, cmpe, t, j0i, j0ii, j1i, j1ii )
    paira0i (j,k,kp,ida,ita,iya,i_ray) = REAL(DLOG10( DABS(j0i ) + epsilon ))
    paira0ii(j,k,kp,ida,ita,iya,i_ray) = REAL(DLOG10( DABS(j0ii) + epsilon ))
  END DO

END DO

RETURN
END SUBROUTINE pairrgn
