SUBROUTINE pair_A_rate( n, jr_min, jr_max, ij_ray, ik_ray, rho, t, ye, &
& r, nx )
!-----------------------------------------------------------------------
!
!    File:         pair_A_rate
...
!  ipairA        : 0, neutrino pair annihilation and production turned off; pair annihilation
!                   subroutines are bypassed; pair annihilation function arrays, if used,
!                   must be zeroed elsewhere
...
!  rhopairxmn    : rho < rhopairxmn: x-neutrino-antineutrino pair annihilation and production omitted
!  rhopairxmx    : rho > rhopairxmx: x-neutrino-antineutrino pair annihilation and production omitted
...
!
!   paf_A(1) = a0w     pafi(1) = da0w/di     pafpi(1,k) = da0w/dpsii(k)
!   paf_A(2) = b0w     pafi(2) = db0w/di     pafpi(2,k) = db0w/dpsii(k)
!   paf_A(3) = c0w     pafi(3) = dc0w/di     pafpi(3,k) = dc0w/dpsii(k)
!   paf_A(4) = a1w     pafi(4) = da1w/di     pafpi(4,k) = da1w/dpsii(k)
!   paf_A(5) = b1w     pafi(5) = db1w/di     pafpi(5,k) = db1w/dpsii(k)
!   paf_A(6) = c1w     pafi(6) = dc1w/di     pafpi(6,k) = dc1w/dpsii(k)
!
!    Subprograms called:
...
USE pair_A_module, ONLY : paf_A, pafd_A, paft_A, pafy_A, pafp0_A, pafp1_A
...
IF ( nnugp(n) == 0  .or.  ipairA == 0 ) RETURN
...
!-----------------------------------------------------------------------
  ...
  IF ( n <  3  .and.  rho(j) < rhopairAemn ) pair_off = .true.
  IF ( n <  3  .and.  rho(j) > rhopairAemx ) pair_off = .true.
  IF ( n == 3  .and.  rho(j) < rhopairAtmn ) pair_off = .true.
  IF ( n == 3  .and.  rho(j) > rhopairAtmx ) pair_off = .true.
  ...
  IF ( pair_off ) THEN
    DO k = 1,nez
      DO i = 1,6
        ...
        pafy_A(i,j,k,n)       = zero
        DO kp = 1,nez
          pafp0_A(i,kp,j,k,n) = zero
          pafp1_A(i,kp,j,k,n) = zero
        END DO ! kp
      END DO ! i
    END DO ! k
    CYCLE ! j loop
...
!-----------------------------------------------------------------------
!  Initialize neutrino-nuclear pair annihilation functions
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!  Get neutrino-nuclear pair annihilation and production kernels.
!
!  f0p and f1p have dimensions of 1 /[ energy**3 length ].
!  zt, asp, c1, gp, and the scf(i) have dimensions of 1 /[ length ].
!----------------------------------------------------------------------

    CALL pair_A_krnl( n, j, ij_ray, ik_ray, k, rho(j), t(j), ye(j), f0a, &
&    f0p, f0ad, f0pd, f0at, f0pt, f0ay, f0py, nez )
...
!-----------------------------------------------------------------------
!  Compute angular correction factors to neutrino-nuclear pair
...
!-----------------------------------------------------------------------
!  Correct neutrino-nuclear pair annihilation kernals
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!  Compute neutrino-nuclear pair production functions.
!
...
!-----------------------------------------------------------------------
      ...
      zlthpd       = zlthpd + ( f0ad(kp) * x0a + f0pd(kp) * ( one - x0a ) ) * w2dw(kp)
      asthpd       = asthpd + x1a  * ( f1ad(kp) - f1pd(kp) )                * w2dw(kp)
      c1d          = c1d    + f0pd(kp) * ( one - x0a )                      * w2dw(kp)
      gthpd        = gthpd  - x1a  * f1pd(kp)                               * w2dw(kp)

      zlthpt       = zlthpt + ( f0at(kp) * x0a + f0pt(kp) * ( one - x0a ) ) * w2dw(kp)
      asthpt       = asthpt + x1a  * ( f1at(kp) - f1pt(kp) )                * w2dw(kp)
      c1t          = c1t    + f0pt(kp) * ( one - x0a )                      * w2dw(kp)
      gthpt        = gthpt  - x1a  * f1pt(kp)                               * w2dw(kp)

      zlthpy       = zlthpy + ( f0ay(kp) * x0a + f0py(kp) * ( one - x0a ) ) * w2dw(kp)
      asthpy       = asthpy + x1a  * ( f1ay(kp) - f1py(kp) )                * w2dw(kp)
      c1y          = c1y    + f0py(kp) * ( one - x0a )                      * w2dw(kp)
      gthpy        = gthpy  - x1a  * f1py(kp)                               * w2dw(kp)

      zlthpp0(kp)  = ( f0a(kp) - f0p(kp) )                                  * w2dw(kp)
      asthpp1(kp)  = ( f1a(kp) - f1p(kp) )                                  * w2dw(kp)
      c1p0(kp)     = - f0p(kp)                                              * w2dw(kp)
      gthpp1(kp)   = - f1p(kp)                                              * w2dw(kp)
...
!-----------------------------------------------------------------------
    ...
    paf_A(1,j,k,n)         = -zlthp
    paf_A(2,j,k,n)         = -asthp/3.d0
    paf_A(3,j,k,n)         =  c1
    paf_A(4,j,k,n)         = -asthp
    paf_A(5,j,k,n)         = -zlthp
    paf_A(6,j,k,n)         =  gthp

    pafd_A(1,j,k,n)        = -zlthpd
    pafd_A(2,j,k,n)        = -asthpd/3.d0
    pafd_A(3,j,k,n)        =  c1d
    pafd_A(4,j,k,n)        = -asthpd
    pafd_A(5,j,k,n)        = -zlthpd
    pafd_A(6,j,k,n)        =  gthpd

    paft_A(1,j,k,n)        = -zlthpt
    paft_A(2,j,k,n)        = -asthpt/3.d0
    paft_A(3,j,k,n)        =  c1t
    paft_A(4,j,k,n)        = -asthpt
    paft_A(5,j,k,n)        = -zlthpt
    paft_A(6,j,k,n)        =  gthpt

    pafy_A(1,j,k,n)        = -zlthpy
    pafy_A(2,j,k,n)        = -asthpy/3.d0
    pafy_A(3,j,k,n)        =  c1y
    pafy_A(4,j,k,n)        = -asthpy
    pafy_A(5,j,k,n)        = -zlthpy
    pafy_A(6,j,k,n)        =  gthpy
...
!-----------------------------------------------------------------------
    ...
    DO kp = 1,nnugp(n)
      ...
      pafp0_A(1,kp,j,k,n)  = -zlthpp0(kp)
      pafp0_A(2,kp,j,k,n)  =  zero
      pafp0_A(3,kp,j,k,n)  =  c1p0(kp)
      pafp0_A(4,kp,j,k,n)  =  zero
      pafp0_A(5,kp,j,k,n)  = -zlthpp0(kp)
      pafp0_A(6,kp,j,k,n)  =  zero

      pafp1_A(1,kp,j,k,n)  =  zero
      pafp1_A(2,kp,j,k,n)  = -asthpp1(kp)/3.
      pafp1_A(3,kp,j,k,n)  =  zero
      pafp1_A(4,kp,j,k,n)  = -asthpp1(kp)
      pafp1_A(5,kp,j,k,n)  =  zero
      pafp1_A(6,kp,j,k,n)  =  gthpp1(kp)
!-----------------------------------------------------------------------
!                    ||||| End loop over kp |||||
!-----------------------------------------------------------------------
