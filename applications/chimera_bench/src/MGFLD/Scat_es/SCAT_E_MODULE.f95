...
!-----------------------------------------------------------------------
!
!  idrse(j,ij_ray,ik_ray), itrse(j,ij_ray,ik_ray)), and iyrse(j,ij_ray,ik_ray))
!   are integers defining the location of log(rho), log(t), and ye for
!   radial zone j on the grid for neutrino-electron scattering functions,
!   i.e.,
!
!     idrse(j,ij_ray,ik_ray)/dgrid < log(rho(j)) < ( idrse(j,ij_ray,ik_ray) + 1 )/dgrid 
!
!     itrse(j,ij_ray,ik_ray)/tgrid <  log(t(j))  < ( itrse(j,ij_ray,ik_ray) + 1 )/tgrid
!
!     0.5 - iyrse(j,ij_ray,ik_ray)/ygrid < ye < 0.5 - ( iyrse(j,ij_ray,ik_ray) + 1 )/ygrid
!
!  The eight grid points surrounding log(rho), log(t), and ye for
!   radial zone j are referred to  as the unit cube j. Neutrino-electron
...
!-----------------------------------------------------------------------
!
!  scte0i(j,k,kp,id,it,iy,ij_ray,ik_ray),
!  scte0ii(j,k,kp,id,it,iy,ij_ray,ik_ray) : arrays of the zero
!   moments of the  neutrino-electron scattering functions as a function
...
!   Interpolations of neutrino-electron scattering functions are computed
!   from this table.
!-----------------------------------------------------------------------
