SUBROUTINE sctnnrate( n, jr_min, jr_max, ij_ray, ik_ray, rho, t, ye, nx )
!-----------------------------------------------------------------------
!
!    File:         sctnnrate
!    Module:       sctnnrate
...
!       neutrino-nucleon inelastic scattering functions and their derivatives
...
!  iscat               : 0, all neutrino scattering processes omitted 1, neutrino scattering processes
!                         not necessarily omitted
!   isctnn             : 0, neutrino-nucleon inelastic scattering omitted
!                        1, neutrino-nucleon inelastic scattering included
...
!
!  scnnf(i,j,k,n)      : neutrino-electron scattering function i
!                       i = 1: a0w, i = 2: b0w, i = 3: c0w,
!                       i = 4: a1w, i = 5: b1w, i = 6: c1w,
!
!  scnnfd(i,j,k,n)     : d(scnf(i,j,k,n))/d(density)
!  scnnft(i,j,k,n)     : d(scnf(i,j,k,n))/d(temperature)
!  scnnfy(i,j,k,n)     : d(scnf(i,j,k,n))/d(electron fraction)
!  scnnfp0(i,kp,j,k,n) : d(scnf(i,j,k,n))/d(psi0(j,kp,n))
!  scnnfp1(i,kp,j,k,n) : d(scnf(i,j,k,n))/d(psi1(j,kp,n))
!
!    Subprograms called:
!  sctnnkrnl           : interpolates the scattering functions
!
!    Include files:
...
USE scat_nn_module, ONLY : scnnf, scnnfd, scnnft, scnnfy, scnnfp0, scnnfp1
...
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: scatnn0       ! zero moment of the NNNS scattering function
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: scatnn1       ! first moment of the NNNS scattering function
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: scatnn0d      ! d(scatnn0)/d(rho)
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: scatnn1d      ! d(scatnn1)/d(rho)
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: scatnn0t      ! d(scatnn0)/d(t)
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: scatnn1t      ! d(scatnn1)/d(t)
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: scatnn0y      ! d(scatnn0)/d(ye)
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: scatnn1y      ! d(scatnn1)/d(ye)

REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: f0in          ! zero moment of the NNNS in scattering function
...
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: f0ot          ! zero moment of the NNNS out scattering function
...
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: f1in          ! first moment of the NNNS in scattering function
...
REAL(KIND=double), ALLOCATABLE, DIMENSION(:) :: f1ot          ! first moment of the NNNS out scattering function
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (' Allocation problem for array ',a10,' in sctnnrate')
 2001 FORMAT (' Deallocation problem for array ',a10,' in sctnnrate')

!-----------------------------------------------------------------------
!-----------------------------------------------------------------------

!-----------------------------------------------------------------------
!  Set neutrino-nucleon inelastic scattering functions to zero if
!     nnugp(n) =  0
!   or
!     isctnn = 0
!   or
...
IF ( nnugp(n) == 0  .or.  iscat  == 0  .or.  isctnn == 0 ) RETURN
...
ALLOCATE (scatnn0(nez), STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn0   '; WRITE (nlog,1001) var_name; END IF
ALLOCATE (scatnn1(nez), STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn1   '; WRITE (nlog,1001) var_name; END IF
ALLOCATE (scatnn0d(nez), STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn0d  '; WRITE (nlog,1001) var_name; END IF
ALLOCATE (scatnn1d(nez), STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn1d  '; WRITE (nlog,1001) var_name; END IF
ALLOCATE (scatnn0t(nez), STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn0t  '; WRITE (nlog,1001) var_name; END IF
ALLOCATE (scatnn1t(nez), STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn1t  '; WRITE (nlog,1001) var_name; END IF
ALLOCATE (scatnn0y(nez), STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn0y  '; WRITE (nlog,1001) var_name; END IF
ALLOCATE (scatnn1y(nez), STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn1y  '; WRITE (nlog,1001) var_name; END IF
...
!-----------------------------------------------------------------------        
!  Set neutrino-nucleon inelastic scattering functions to zero if rho
!   outside specified boundaries.
!-----------------------------------------------------------------------

  sctnn_off                       = .false.
  IF ( n < 3   .and.  rho(j) < rhosctnnemn ) sctnn_off = .true.
  IF ( n < 3   .and.  rho(j) > rhosctnnemx ) sctnn_off = .true.
  IF ( n == 3  .and.  rho(j) < rhosctnntmn ) sctnn_off = .true.
  IF ( n == 3  .and.  rho(j) > rhosctnntmx ) sctnn_off = .true.
  ...
  IF ( sctnn_off ) THEN
    DO k = 1,nez
      DO i = 1,6
        ...
        scnnfy(i,j,k,n)           = zero
        DO kp = 1,nez
          scnnfp0(i,kp,j,k,n)     = zero
          scnnfp1(i,kp,j,k,n)     = zero
        END DO
      END DO
    END DO
    CYCLE outer
...
!-----------------------------------------------------------------------
    ...
    CALL sctnnkrnl( n, j, ij_ray, ik_ray, k, rho(j), t(j), ye(j), scatnn0, &
& scatnn1, scatnn0d, scatnn1d, scatnn0t, scatnn1t, scatnn0y, scatnn1y, nez )
...
!-----------------------------------------------------------------------
    ...
    DO kp = 1,nnugp(n)
      ...
      IF ( kp < k ) THEN
        ...
        f0ot(kp)   = scatnn0(kp)
        f0in(kp)   = scatnn0(kp)                                 * rinv
        f0otd(kp)  = scatnn0d(kp)
        f0ind(kp)  = scatnn0d(kp)                                * rinv
        f0ott(kp)  = scatnn0t(kp)
        f0int(kp)  = scatnn0t(kp)                                * rinv
        f0oty(kp)  = scatnn0y(kp)
        f0iny(kp)  = scatnn0y(kp)                                * rinv
        f1ot(kp)   = scatnn1(kp)
        f1in(kp)   = scatnn1(kp)                                 * rinv
        f1otd(kp)  = scatnn1d(kp)
        f1ind(kp)  = scatnn1d(kp)                                * rinv
        f1ott(kp)  = scatnn1t(kp)
        f1int(kp)  = scatnn1t(kp)                                * rinv
        f1oty(kp)  = scatnn1y(kp)
        f1iny(kp)  = scatnn1y(kp)                                * rinv
      ...
      ELSE IF ( kp == k ) THEN
        ...
        f0in(kp)   = scatnn0 (kp)
        f0otd(kp)  = scatnn0d(kp)
        f0ind(kp)  = scatnn0d(kp)
        f0ott(kp)  = scatnn0t(kp)
        f0int(kp)  = scatnn0t(kp)
        f0oty(kp)  = scatnn0y(kp)
        f0iny(kp)  = scatnn0y(kp)
        f1ot(kp)   = scatnn1 (kp)
        f1in(kp)   = scatnn1 (kp)
        f1otd(kp)  = scatnn1d(kp)
        f1ind(kp)  = scatnn1d(kp)
        f1ott(kp)  = scatnn1t(kp)
        f1int(kp)  = scatnn1t(kp)
        f1oty(kp)  = scatnn1y(kp)
        f1iny(kp)  = scatnn1y(kp)
      ...
      ELSE IF ( kp > k ) THEN
        ...
        f0in(kp)   = scatnn0(kp)
        f0ot(kp)   = scatnn0(kp)                                 * rinv
        f0ind(kp)  = scatnn0d(kp)
        f0otd(kp)  = scatnn0d(kp)                                * rinv
        f0int(kp)  = scatnn0t(kp)
        f0ott(kp)  = scatnn0t(kp)                                * rinv
        f0iny(kp)  = scatnn0y(kp)
        f0oty(kp)  = scatnn0y(kp)                                * rinv
        f1in(kp)   = scatnn1(kp)
        f1ot(kp)   = scatnn1(kp)                                 * rinv
        f1ind(kp)  = scatnn1d(kp)
        f1otd(kp)  = scatnn1d(kp)                                * rinv
        f1int(kp)  = scatnn1t(kp)
        f1ott(kp)  = scatnn1t(kp)                                * rinv
        f1iny(kp)  = scatnn1y(kp)
        f1oty(kp)  = scatnn1y(kp)                                * rinv
...
!
!  scnnf(1) = a0w   scnnfi(1) = da0w/di   scnnfpi(1,k) = da0w/dpsii(k)
!  scnnf(2) = b0w   scnnfi(2) = db0w/di   scnnfpi(2,k) = db0w/dpsii(k)
!  scnnf(3) = c0w   scnnfi(3) = dc0w/di   scnnfpi(3,k) = dc0w/dpsii(k)
!  scnnf(4) = a1w   scnnfi(4) = da1w/di   scnnfpi(4,k) = da1w/dpsii(k)
!  scnnf(5) = b1w   scnnfi(5) = db1w/di   scnnfpi(5,k) = db1w/dpsii(k)
!  scnnf(6) = c1w   scnnfi(6) = dc1w/di   scnnfpi(6,k) = dc1w/dpsii(k)
!-----------------------------------------------------------------------

    scnnf(1,j,k,n)  = -zt
    scnnf(2,j,k,n)  = -asp/3.d0
    scnnf(3,j,k,n)  = c1
    scnnf(4,j,k,n)  = -asp
    scnnf(5,j,k,n)  = -zt
    scnnf(6,j,k,n)  = gp
...
!-----------------------------------------------------------------------
    ...
    DO kp = 1,nnugp(n)
      ...
      scnnfp0(1,kp,j,k,n) = -ztp0(kp)
      scnnfp0(2,kp,j,k,n) = zero
      scnnfp0(3,kp,j,k,n) = c1p0(kp)
      scnnfp0(4,kp,j,k,n) = zero
      scnnfp0(5,kp,j,k,n) = -ztp0(kp)
      scnnfp0(6,kp,j,k,n) = zero

      scnnfp1(1,kp,j,k,n) = zero
      scnnfp1(2,kp,j,k,n) = -aspp1(kp)/3.d0
      scnnfp1(3,kp,j,k,n) = zero
      scnnfp1(4,kp,j,k,n) = -aspp1(kp)
      scnnfp1(5,kp,j,k,n) = zero
      scnnfp1(6,kp,j,k,n) = gpp1(kp)
...
DEALLOCATE (scatnn0, STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn0   '; WRITE (nlog,2001) var_name; END IF
DEALLOCATE (scatnn1, STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn1   '; WRITE (nlog,2001) var_name; END IF
DEALLOCATE (scatnn0d, STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn0d  '; WRITE (nlog,2001) var_name; END IF
DEALLOCATE (scatnn1d, STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn1d  '; WRITE (nlog,2001) var_name; END IF
DEALLOCATE (scatnn0t, STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn0t  '; WRITE (nlog,2001) var_name; END IF
DEALLOCATE (scatnn1t, STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn1t  '; WRITE (nlog,2001) var_name; END IF
DEALLOCATE (scatnn0y, STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn0y  '; WRITE (nlog,2001) var_name; END IF
DEALLOCATE (scatnn1y, STAT = istat)
  IF ( istat /= 0 ) THEN; var_name = 'scatnn1y  '; WRITE (nlog,2001) var_name; END IF
