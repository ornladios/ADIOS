SUBROUTINE remap_psi_y( ngeom, jmin, jmax, ji_ray, jk_ray, ny, nez, nnu, &
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (' Allocation problem for array ',a10,' in remap_psi_y')
 2001 FORMAT (' Deallocation problem for array ',a10,' in remap_psi_y')
...
ALLOCATE (dvol0_inv(ny+12), STAT = istat)
...
DO nn = 1,nnu
  ...
  DO k = 1,nnugp(nn)
    ...
    psi(1:jmax+12)   = psi0_re(1:jmax+12,k,nn)

    CALL parabola( nmin-3, nmax+3, ntot, zparay, psi, dpsi, psi6, psil, &
&    dm, 0, 0, ngeom )

    dpsi_re(1:jmax+12,k,nn) = dpsi(1:jmax+12)
    psi6_re(1:jmax+12,k,nn) = psi6(1:jmax+12)
    psil_re(1:jmax+12,k,nn) = psil(1:jmax+12)
...
DO n = nmin,nmax
  ...
  DO k = 1,nnugpmx
    DO nn = 1,nnu
      IF ( nnugp(nn)  ==  0 ) CYCLE
      IF ( j_shock(j) == 1  .or.  ( .not. l_diff_off ) ) THEN
