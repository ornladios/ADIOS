SUBROUTINE remap_comp_z( ngeom, kmin, kmax, ki_ray, kj_ray, nz, nnc )
!-----------------------------------------------------------------------
!
!    File:         remap_comp_z
!    Module:       remap_comp_z
...
!  kmin        : minimum y-array index
!  kmax        : maximim y-array index
...
USE eos_snc_z_module, ONLY: nse, xn, a_nuc_rep, z_nuc_rep, be_nuc_rep, &
...
INTEGER, INTENT(in)                            :: kmax         ! maximim y-array index
...
!-----------------------------------------------------------------------
 ...
 1001 FORMAT (' Allocation problem for array ',a10,' in remap_comp_z')
 2001 FORMAT (' Deallocation problem for array ',a10,' in remap_comp_z')
...
nse_zn                 = 1
DO iz = kmin,kmax
  IF ( nse(iz,kj_ray,ki_ray) == 0 ) THEN
    nse_zn             = 0
    EXIT
  END IF
END DO
...
ALLOCATE (fluxcmp(nz+12,nnc), STAT = istat)
...
DO n = nmin, nmax
  ...
  xn_t(1:n_nucp1)          = xn(iz,1:n_nucp1)

  CALL eos_nnse_e( iz, r(n), temp(n), ye(n), xn_t, nnc, a_nuc_rep(iz), z_nuc_rep(iz), &
&  be_nuc_rep(iz), e_ph, e_elec, e_drip, e_hvy, e_bind, e_no_bind, e_total )
...
DO nc = 1,n_nucp1
  ...
  cmp(kmin:ntot)           = comp(kmin:ntot,nc)

  CALL parabola( nmin-3, nmax+3, ntot, zparay, cmp, dcmp, cmp6, cmpl,   &
&  dum, 0, 0 )

  dcomp(kmin:ntot,nc)      = dcmp(kmin:ntot)
  comp6(kmin:ntot,nc)      = cmp6(kmin:ntot)
  compl(kmin:ntot,nc)      = cmpl(kmin:ntot)
...
!
!       n-1 = iz is material on left; n = iz+1 is the material on right
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
!        \\\\\ nse(iz) = 0 --> mse(iz+1) = 0 or nse(iz+1) = 1 /////
!        /////        nse(iz) = 1 --> mse(iz+1) = 0          \\\\\
!
!  If nse(iz = n-7,kj_ray,ki_ray) = 0 or 1, composition profile in zone iz
!   (zone n-1) is integrated over causal domain to be transferred out of
!   zone iz.
!   If  nse(iz+1) = 0 this composition is transferred into zone iz+1 (zone n).
!   If nse(iz+1) = 1, rho, e, and ye are transferred into zone iz+1 in
!   subroutine remap_y.
!-----------------------------------------------------------------------
...
!------------------------------------------------------------------------
    ...
    IF ( nse(iz,kj_ray,ki_ray) /= 1  .or.  nse(iz+1,kj_ray,ki_ray) /= 1 ) THEN
...
!-----------------------------------------------------------------------
      ...
      a_nuc_rep_t          = a_nuc_rep(iz)
      z_nuc_rep_t          = z_nuc_rep(iz)
      be_nuc_rep_t         = be_nuc_rep(iz)

      CALL eos_nnse_e( iz, r(n), temp(n), ye(n), xn_t, nnc, a_nuc_rep_t ,z_nuc_rep_t, &
...
!-----------------------------------------------------------------------
    ...
    END IF ! nse(iz,kj_ray,ki_ray) /= 1  .or.  nse(iz+1,kj_ray,ki_ray) /= 1
...
!-----------------------------------------------------------------------
!  If a_nuc_rep(iz) < 1, compl(k,n_nucp1) should be zero, but will be
...
!-----------------------------------------------------------------------
    ...
    IF ( a_nuc_rep(iz) < 1.d0 ) fluxcmp(n,n_nucp1) = zero
    flux_n_rep(n)          = fluxcmp(n,n_nucp1)/( a_nuc_rep(iz) + epsilon )
    flux_z_rep(n)          = z_nuc_rep(iz)  * flux_n_rep(n)
    flux_b_rep(n)          = be_nuc_rep(iz) * flux_n_rep(n)
...
!
!       n-1 = iz-1 is material on left; n = iz is the material on right
!
!-----------------------------------------------------------------------
...
!        /////        nse(iz) = 1 --> mse(iz-1) = 0           \\\\\
!
!  If nse(iz = n-6,kj_ray,ki_ray) = 0 or 1, composition profile in zone iz
!   (zone n) is integrated over causal domain to be transferred out of
!   zone iz.
!  If  nse(iz-1) = 0 the composition from iz is transferred into zone iz-1.
!  If nse(iz-1) = 1, rho, e, and ye are transferred into zone iz-1 in
!   subroutine remap_y.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      CALL eos_nnse_e( iz, r(n), temp(n), ye(n), xn_t, nnc, a_nuc_rep_t, &
...
!-----------------------------------------------------------------------
    ...
    END IF ! nse(iz-1,kj_ray,ki_ray) == 0
...
DO n = nmin, nmax
  ...
  n0_rep(n)                = comp(n,n_nucp1) * dm(n)/( a_nuc_rep(iz) + epsilon )
  n_rep(n)                 = n0_rep(n)                 + flux_n_rep(n) - flux_n_rep(n+1)
  z_rep(n)                 = z_nuc_rep(iz)  * n0_rep(n) + flux_z_rep(n) - flux_z_rep(n+1)
  b_rep(n)                 = be_nuc_rep(iz) * n0_rep(n) + flux_b_rep(n) - flux_b_rep(n+1)

  comp(n,1:n_nucp1)        = ( comp(n,1:n_nucp1) * dm(n) + fluxcmp(n,1:n_nucp1) - fluxcmp(n+1,1:n_nucp1) ) * dm0(n)

  IF ( comp(n,n_nucp1) > epsilon ) THEN
    a_nuc_rep  (iz)        = comp(n,n_nucp1)/( n_rep(n) * dm0(n) + epsilon )
    z_nuc_rep (iz)         = z_rep(n)       /( n_rep(n) + epsilon )
    be_nuc_rep(iz)         = b_rep(n)       /( n_rep(n) + epsilon )
  END IF
...
DO n = nmin, nmax
  iz                       = n - 6
  ye_tot                   = SUM( xn(iz,1:nuc_number) * z_nuc(1:nuc_number)/( a_nuc(1:nuc_number) + epsilon ) )
  ye_tot                   = ye_tot + xn(iz,n_nucp1) * z_nuc_rep(iz)/( a_nuc_rep(iz) + epsilon )
  ye(n)                    = ye_tot
  ye_r(n)                  = ye(n)
END DO ! n = nminc,nmax
