...
& r_e_rms, d_e_rms, nenuplt1, nenuplt2, d_omega, data_path
...
REAL(KIND=double), DIMENSION(6,nnu,ij_ray_dim,ik_ray_dim)                   :: send_buf   ! send buffer for gathering data from processors
REAL(KIND=double), DIMENSION(6,nnu,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z) :: recv_buf   ! receive buffer for gathering data from processors
...
IF ( nenu >= nenu_test ) THEN
  lprint           = .true.
  IF ( ij_ray /= ij_ray_dim  .or.  ij_ray /= ij_ray_dim ) nenu = n_zero
END IF
...
!-----------------------------------------------------------------------
!
!     \\\\\ ANGULARLY AVERAGE NEUTRINO RMS ENERGIES AND EDIT /////
!
!-----------------------------------------------------------------------
...
c_gath_recv         = 6 * nnu * ij_ray_dim * ik_ray_dim
c_gath_send         = 6 * nnu * ij_ray_dim * ik_ray_dim
...
!-----------------------------------------------------------------------
  ...
  DO m = 0,n_proc-1
    ...
    DO k = 1,ik_ray_dim
      ...
      DO j = 1,ij_ray_dim
        jsk         = mj * ij_ray_dim + j
        enuv1r_a  (:,jsk,ksk) = recv_buf(1,:,j,k,m+1)
        enuv2r_a  (:,jsk,ksk) = recv_buf(2,:,j,k,m+1)
        enuv3r_a  (:,jsk,ksk) = recv_buf(3,:,j,k,m+1)
        enuv1rho_a(:,jsk,ksk) = recv_buf(4,:,j,k,m+1)
        enuv2rho_a(:,jsk,ksk) = recv_buf(5,:,j,k,m+1)
        enuv3rho_a(:,jsk,ksk) = recv_buf(6,:,j,k,m+1)
...
!-----------------------------------------------------------------------
!  End editting of mean energies at r_e_rms.
!-----------------------------------------------------------------------
