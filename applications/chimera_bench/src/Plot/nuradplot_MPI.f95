...
& rho_nurad, r_nurad, d_omega, data_path
...
REAL(KIND=double), DIMENSION(8,nez,nnu,ij_ray_dim,ik_ray_dim)                   :: send_buf   ! send buffer for gathering data from processors
REAL(KIND=double), DIMENSION(8,nez,nnu,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z) :: recv_buf   ! receive buffer for gathering data from processors
...
c_gath_recv           = 8 * nez * nnu * ij_ray_dim * ik_ray_dim
c_gath_send           = 8 * nez * nnu * ij_ray_dim * ik_ray_dim
...
!-----------------------------------------------------------------------
  ...
  DO m = 0,n_proc-1
    ...
    DO k = 1,ik_ray_dim
      ...
      DO j = 1,ij_ray_dim
        jsk           = mj * ij_ray_dim + j
        nu_r_a    (:,:,jsk,ksk) = recv_buf(1,:,:,j,k,m+1)
        nu_rt_a   (:,:,jsk,ksk) = recv_buf(2,:,:,j,k,m+1)
        nu_rho_a  (:,:,jsk,ksk) = recv_buf(3,:,:,j,k,m+1)
        nu_rhot_a (:,:,jsk,ksk) = recv_buf(4,:,:,j,k,m+1)
        unu_r_a   (:,:,jsk,ksk) = recv_buf(5,:,:,j,k,m+1)
        unu_rt_a  (:,:,jsk,ksk) = recv_buf(6,:,:,j,k,m+1)
        unu_rho_a (:,:,jsk,ksk) = recv_buf(7,:,:,j,k,m+1)
        unu_rhot_a(:,:,jsk,ksk) = recv_buf(8,:,:,j,k,m+1)
