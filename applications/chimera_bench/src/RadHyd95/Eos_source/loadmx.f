...
c     CALL GETFNM(FNAME2,FNML2,'Enter ASCII boundary fname:',27)
          if ( loaddata ) return
      FNAME1        = '../../Equation_of_State/Eos_lsu/max180.atb'
      FNML1         = 42
      FNAME2        = '../../Equation_of_State/Eos_lsu/bd180.atb'
      FNML2         = 41
      loaddata      = .true.
C
C
...
C
      WRITE(nlog,*)
      WRITE(nlog,*) '<<LOADMX:  MAXWELL CON. TABLE IS INITIALIZED>>'
      WRITE(nlog,*)
C
C
C
...
C
      WRITE(nlog,*)
      WRITE(nlog,*) '<<LOADMX:  BOUNDARY TABLE IS INITIALIZED>>'
      WRITE(nlog,*)
C
C
C
...
C
      WRITE(nlog,*)
      WRITE(nlog,*) '<<LOADMX:  SKYRME PARAMETERS FOR THIS RUN ARE:>>'
      WRITE(nlog,*) 'ABCD: ',AA,BB,CC,DD,SCRDD3
      WRITE(nlog,*) ' Satur. density, symmetry engy, & compression
     1 mod.:'
      WRITE(nlog,*) N_SM, SYMM_M, COMP_M
      WRITE(nlog,*) N_SB, SYMM_B, COMP_B
      WRITE(nlog,*) ' Binding engy, surf. symm. engy, & surface
     1 tension:'
      WRITE(nlog,*) BINDEM,SYM_SM,SIG_SM
      WRITE(nlog,*) BINDEB,SYM_SB,SIG_SB
C
      WRITE(nlog,*)
C
C
...
C
      WRITE(nlog,*)
      WRITE(nlog,*) '<<LOADMX: FERMI INTEGRAL TABLES ARE INITIALIZED>>'
      WRITE(nlog,*)
C
C
 999  RETURN
