SUBROUTINE setup_initial_hydro( imin, imax, ij_ray, ik_ray, ij_ray_dim, &
& ik_ray_dim, nx, ny, nz, nnc, rhop, tp, yep, ye_ip, rp, drp, up, xnp, &
& be_nuc_repp, a_nuc_repp, z_nuc_repp, aesv_c )
!-----------------------------------------------------------------------
!
!    File:         setup_initial_hydro
!    Module:       setup_initial_hydro
!    Type:         Subprogram
...
!
!    Purpose:
!      To compute, from the initial problem configuration, the quantities
!       needed to begin the execution of the simulation. In particular,
!
...
!  boundary_module, convect_module, edit_module, eos_snc_x_module,
!  mdl_cnfg_module, nucbrn_module, shock_module
!     
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
  ...
  101 FORMAT (' Variable initialization complete in setup_initial_hydro')
  103 FORMAT (' Velocity boundary conditions set (if option selected) in setup_initial_hydro')
  105 FORMAT (' Variable transfer complete in setup_initial_hydro')
  107 FORMAT (' pblmst1 called in setup_initial_hydro')
  109 FORMAT (' Equation of state tables are being filled in setup_initial_hydro')
  111 FORMAT (' Equation of state quantities are being interpolated in setup_initial_hydro')
  113 FORMAT (' Adiabatic exponents are being computed in setup_initial_hydro')
  115 FORMAT (' Equation of state tables have been loaded in setup_initial_hydro')
  117 FORMAT (' Composition mass fractions have been loaded for NSE material in setup_initial_hydro')
  119 FORMAT (' Convection variables have been initialized in setup_initial_hydro')
...
RETURN
END SUBROUTINE setup_initial_hydro
