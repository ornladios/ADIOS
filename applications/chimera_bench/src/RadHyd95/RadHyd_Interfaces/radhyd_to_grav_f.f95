SUBROUTINE radhyd_to_grav_f( nx, ny, nz, ij_ray_dim, ik_ray_dim )
!-----------------------------------------------------------------------
!
!    File:         radhyd_to_grav_f
!    Module:       radhyd_to_grav_f
!    Type:         Subprogram
...
!
!    Purpose:
!      To compute the gravitational potentials and accelerations after a
!        Lagrangian update and ramap return and store them in radial_ray_module
!
!    Subprograms called:
!        none
!
...
USE radial_ray_module, ONLY : imin, imax, x_ef, x_cf, dx_cf, y_ef, y_cf, &
& dy_cf, z_ef, dz_cf, rho_c, vx_bar, v2_bar, rhobar, e_bar, e_nu_c_bar,  &
& f_nu_e_bar, p_bar, agr_e, agr_c, agr_e_r, agr_c_r, grav_x_c, grav_y_c, &
& grav_z_c, grav_pot_c, gtot_pot_c, grav_x_e, grav_y_e, grav_z_e,        &
& grav_pot_e, gtot_pot_e, unu_c, unub_c, dunu_c, unue_e, unube_e, dunue_e
...
r  (imin:imax+1)                = x_ef   (imin:imax+1)

dmrst(jr_min:jr_max)            = frpi * dx_cf(imin:imax) * ( r(imin:imax) * r(imin+1:imax+1) &
&                               + dx_cf(imin:imax) * dx_cf(imin:imax) * third ) * rhobar(imin:imax)
...
IF ( i_grav == 1 ) THEN
  ...
  IF ( irelhy == 0 ) THEN
    ...
    CALL grav_Newton( imin, imax, x_ef, x_cf, dx_cf, rhobar, gpot_e, gpot_c, &
...
!-----------------------------------------------------------------------
  ...
  ELSE IF ( irelhy == 1 ) THEN
    ...
    CALL greff( imin, imax, x_ef, x_cf, dx_cf, rhobar, e_bar, e_nu_c_bar,   &
...
!-----------------------------------------------------------------------
  ...
  IF ( irelhy == 0 ) THEN
    ...
    CALL grav_Newton( imin, imax, x_ef, x_cf, dx_cf, rhobar, gpot_e, &
&    gpot_c, gforce_e, gforce_c, gpot_dis_e, gpot_dis_c )

    mode                        = 0
    
    CALL poisson( mode, imin, imax, nx, ij_ray_dim, ik_ray_dim, ny, nz,  & 
&    x_ef, x_cf, dx_cf, y_ef, y_cf, dy_cf, z_ef, dz_cf, rho_c, grav_x_c, &
...
!-----------------------------------------------------------------------
!
!            \\\\\ COMPUTE PN GR NONSPHERICAL GRAVITY /////
!
!-----------------------------------------------------------------------

  ELSE IF ( irelhy == 1 ) THEN

    CALL grav_Newton( imin, imax, x_ef, x_cf, dx_cf, rhobar, gpotn_e, &
...
RETURN
END SUBROUTINE radhyd_to_grav_f
