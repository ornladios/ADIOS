SUBROUTINE radhyd_to_monitor_MPI( ij_ray_dim, ik_ray_dim, nx, ny, nz, nez, &
& nnu, nnc )
!-----------------------------------------------------------------------
!
!    File:         radhyd_to_monitor_MPI
!    Module:       radhyd_to_monitor_MPI
!    Type:         Subprogram
...
REAL(KIND=double), DIMENSION(jmax,kmax)      :: u_ge_tot_a      ! total NT gravitational potential energy of ij_ray, ik_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: u_ie_tot_a      ! total internal energy of ij_ray, ik_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: u_ke_tot_a      ! total NT kinetic energy of ij_ray, ik_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: u_ne_tot_a      ! total NT neutrino energy of ij_ray, ik_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: u_ke_x_tot_a    ! total NT x-kinetic energy of i_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: u_ke_y_tot_a    ! total NT y-kinetic energy of i_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: u_ke_z_tot_a    ! total NT z-kinetic energy of i_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: e_rad_a         ! the cumulative material energy entering ij_ray, ik_ray grid (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: radtot_a        ! energy radiated by neutrinos in ij_ray, ik_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: u_tot_a         ! total energy of ij_ray, ik_ray (ergs)
REAL(KIND=double), DIMENSION(jmax,kmax)      :: elecn_a         ! total electron number of ij_ray, ik_ray
REAL(KIND=double), DIMENSION(jmax,kmax)      :: elec_rad_a      ! the net number of electrons advected into the ij_ray, ik_ray grid
REAL(KIND=double), DIMENSION(jmax,kmax)      :: nnucr_enu_a     ! the net number of e-type neutrinos currently residing in ij_ray, ik_ray grid
REAL(KIND=double), DIMENSION(jmax,kmax)      :: nnucr_enubar_a  ! the net number of e-type anatineutrinos currently residing in ij_ray, ik_ray grid
REAL(KIND=double), DIMENSION(jmax,kmax)      :: nnurad_enu_a    ! the cumulative number of e-type neutrinos emitted by the ij_ray, ik_ray grid
REAL(KIND=double), DIMENSION(jmax,kmax)      :: nnurad_enubar_a ! the cumulative number of e-type antineutrinos emitted by the ij_ray, ik_ray grid
REAL(KIND=double), DIMENSION(jmax,kmax)      :: totlpn_a        ! total number of leptons emitted or residing in ij_ray, ik_ray
...
REAL(KIND=double), DIMENSION(17,ij_ray_dim,ik_ray_dim)                   :: send ! total number of leptons emitted or residing in ij_ray, ik_ray
REAL(KIND=double), DIMENSION(17,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z) :: recv ! total number of leptons emitted or residing in ij_ray, ik_ray
...
&  radtot       u_tot'/)
 1003 FORMAT (2x,i7,2x,12(es13.5))
...
ELSE ! jmax /= jmin  or  kmax /= kmin
  ...
  DO ik_ray = 1,ik_ray_dim
    DO ij_ray = 1,ij_ray_dim
      ...
      send( 1,ij_ray,ik_ray) = u_ge_tot     (ij_ray,ik_ray)
      send( 2,ij_ray,ik_ray) = u_ie_tot     (ij_ray,ik_ray)
      send( 3,ij_ray,ik_ray) = u_ke_tot     (ij_ray,ik_ray)
      send( 4,ij_ray,ik_ray) = u_ne_tot     (ij_ray,ik_ray)
      send( 5,ij_ray,ik_ray) = u_ke_x_tot   (ij_ray,ik_ray)
      send( 6,ij_ray,ik_ray) = u_ke_y_tot   (ij_ray,ik_ray)
      send( 7,ij_ray,ik_ray) = u_ke_z_tot   (ij_ray,ik_ray)
      send( 8,ij_ray,ik_ray) = e_rad        (ij_ray,ik_ray)
      send( 9,ij_ray,ik_ray) = radtot       (ij_ray,ik_ray)
      send(10,ij_ray,ik_ray) = u_tot        (ij_ray,ik_ray)
      send(11,ij_ray,ik_ray) = elecn        (ij_ray,ik_ray)
      send(12,ij_ray,ik_ray) = elec_rad     (ij_ray,ik_ray)
      send(13,ij_ray,ik_ray) = nnucr_enu    (ij_ray,ik_ray)
      send(14,ij_ray,ik_ray) = nnucr_enubar (ij_ray,ik_ray)
      send(15,ij_ray,ik_ray) = nnurad_enu   (ij_ray,ik_ray)
      send(16,ij_ray,ik_ray) = nnurad_enubar(ij_ray,ik_ray)
      send(17,ij_ray,ik_ray) = totlpn       (ij_ray,ik_ray)

    END DO ! ij_ray = 1,ij_ray_dim
  END DO ! ik_ray = 1,ik_ray_dim

  c_gath_send            = 17 * ij_ray_dim * ik_ray_dim
  c_gath_recv            = 17 * ij_ray_dim * ik_ray_dim

  CALL MPI_GATHER( send, c_gath_send, MPI_DOUBLE_PRECISION, recv, c_gath_recv, &
  ...
  IF ( myid == 0 ) THEN
    ...
    DO m = 0,n_proc-1
      ...
      DO k = 1,ik_ray_dim
        ...
        DO j = 1,ij_ray_dim
          jsk               = mj * ij_ray_dim + j
          u_ge_tot_a     (jsk,ksk) = recv( 1,j,k,m+1)
          u_ie_tot_a     (jsk,ksk) = recv( 2,j,k,m+1)
          u_ke_tot_a     (jsk,ksk) = recv( 3,j,k,m+1)
          u_ne_tot_a     (jsk,ksk) = recv( 4,j,k,m+1) 
          u_ke_x_tot_a   (jsk,ksk) = recv( 5,j,k,m+1)
          u_ke_y_tot_a   (jsk,ksk) = recv( 6,j,k,m+1)
          u_ke_z_tot_a   (jsk,ksk) = recv( 7,j,k,m+1)
          e_rad_a        (jsk,ksk) = recv( 8,j,k,m+1)
          radtot_a       (jsk,ksk) = recv( 9,j,k,m+1)
          u_tot_a        (jsk,ksk) = recv(10,j,k,m+1)
          elecn_a        (jsk,ksk) = recv(11,j,k,m+1)
          elec_rad_a     (jsk,ksk) = recv(12,j,k,m+1)
          nnucr_enu_a    (jsk,ksk) = recv(13,j,k,m+1)
          nnucr_enubar_a (jsk,ksk) = recv(14,j,k,m+1)
          nnurad_enu_a   (jsk,ksk) = recv(15,j,k,m+1)
          nnurad_enubar_a(jsk,ksk) = recv(16,j,k,m+1)
          totlpn_a       (jsk,ksk) = recv(17,j,k,m+1)
    ...
    u_ge_tot_t           = SUM( u_ge_tot_a     (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    u_ie_tot_t           = SUM( u_ie_tot_a     (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    u_ke_tot_t           = SUM( u_ke_tot_a     (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    u_ne_tot_t           = SUM( u_ne_tot_a     (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    u_ke_x_tot_t         = SUM( u_ke_x_tot_a   (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    u_ke_y_tot_t         = SUM( u_ke_y_tot_a   (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    u_ke_z_tot_t         = SUM( u_ke_z_tot_a   (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    e_rad_t              = SUM( e_rad_a        (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    radtot_t             = SUM( radtot_a       (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    u_tot_t              = SUM( u_tot_a        (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    elecn_t              = SUM( elecn_a        (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    elec_rad_t           = SUM( elec_rad_a     (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    nnucr_enu_t          = SUM( nnucr_enu_a    (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    nnucr_enubar_t       = SUM( nnucr_enubar_a (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    nnurad_enu_t         = SUM( nnurad_enu_a   (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    nnurad_enubar_t      = SUM( nnurad_enubar_a(jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
    totlpn_t             = SUM( totlpn_a       (jmin:jmax,kmin:kmax) * d_omega_n(jmin:jmax,kmin:kmax) )
...
RETURN
END SUBROUTINE radhyd_to_monitor_MPI
