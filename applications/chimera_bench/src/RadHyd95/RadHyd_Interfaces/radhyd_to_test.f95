SUBROUTINE radhyd_to_test( i_ray_dim, nx, ny, nez, nnu, nnc, i_flag )
!-----------------------------------------------------------------------
!
!    File:         radhyd_to_test
!    Module:       radhyd_to_test
!    Type:         Subprogram
...
& rho_c, t_ci, t_c, ye_ci, ye_c, rhobar, x_ef, u_c, psi0_c, psi1_e, dtnph, &
& time, ncycle, xn_c, be_nuc_rep_c, a_nuc_rep_c, z_nuc_rep_c, nse_c, &
& u_ge_tot(i_ray), u_ie_tot(i_ray), u_ke_tot(i_ray), u_ne_tot(i_ray), &
& e_rad(i_ray), radtot(i_ray), u_tot(i_ray), elecn(i_ray), elec_rad(i_ray), &
& nnucr_enu(i_ray), nnucr_enubar(i_ray), nnurad_enu(i_ray), nnurad_enubar(i_ray), &
& totlpn(i_ray) )
