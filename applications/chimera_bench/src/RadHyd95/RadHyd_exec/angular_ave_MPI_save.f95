...
REAL(KIND=double), DIMENSION(nx,ij_ray_dim,ij_ray_dim) :: v2     ! density of radial zone i, angular zone j (g cm^{-3})
...
REAL(KIND=double), DIMENSION(9*nx+1,ij_ray_dim,ij_ray_dim) :: send_buf2   ! send buffer for gathering data from processors
REAL(KIND=double), DIMENSION(9*nx+1,ny,nz)     :: recv_buf2      ! receive buffer for gathering data from processors
...
DO k = 1,ik_ray_dim
  DO j = 1,ij_ray_dim
    send_buf2(1:nx,j,k)          = rho_c (1:nx,  j,k)
    send_buf2(nx+1:2*nx,j,k)     = aesv_c(1:nx,1,j,k)
    send_buf2(2*nx+1:3*nx,j,k)   = aesv_c(1:nx,2,j,k)
    send_buf2(3*nx+1:4*nx,j,k)   = u_c   (1:nx,  j,k)
    send_buf2(4*nx+1:5*nx,j,k)   = v_c   (1:nx,  j,k)
    send_buf2(5*nx+1:6*nx,j,k)   = w_c   (1:nx,  j,k)
    send_buf2(6*nx+1:7*nx,j,k)   = v2    (1:nx,  j,k)
    send_buf2(7*nx+1:8*nx,j,k)   = e_nu_c(1:nx,  j,k)
    send_buf2(8*nx+1:9*nx+1,j,k) = f_nu_e(1:nx+1,j,k)
  END DO ! ij_ray_dim
END DO ! ik_ray_dim
...
!-----------------------------------------------------------------------
  ...
  DO k = 1,nz
    DO j = 1,ny
      rho_all (1:nx,  j,k)       = recv_buf2(1:nx,         j,k)
      p_all   (1:nx,  j,k)       = recv_buf2(nx+1:2*nx,    j,k)
      e_all   (1:nx,  j,k)       = recv_buf2(2*nx+1:3*nx,  j,k)
      vx_all  (1:nx,  j,k)       = recv_buf2(3*nx+1:4*nx,  j,k)
      vy_all  (1:nx,  j,k)       = recv_buf2(4*nx+1:5*nx,  j,k)
      vz_all  (1:nx,  j,k)       = recv_buf2(5*nx+1:6*nx,  j,k)
      v2_all  (1:nx,  j,k)       = recv_buf2(6*nx+1:7*nx,  j,k)
      e_nu_all(1:nx,  j,k)       = recv_buf2(7*nx+1:8*nx,  j,k)
      f_nu_all(1:nx+1,j,k)       = recv_buf2(8*nx+1:9*nx+1,j,k)
    END DO ! j = 1,ny
  END DO ! k = 1,nz
...
!-----------------------------------------------------------------------
  ...
  ELSE ! jmax /= jmin and/or kmax /= kmin
    DO i = imin,imax
      f_nu_e_bar(i)              = SUM( f_nu_e_bar(i,jmin:jmax,kmin:kmax) * d_omega(jmin:jmax,kmin:kmax) )/frpi
    END DO ! i = imin,imax
  END IF ! jmax == jmin and kmin == kmax
