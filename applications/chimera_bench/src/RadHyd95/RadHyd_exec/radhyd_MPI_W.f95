...
IF ( myid >= ml  .and.  myid <= mu ) WRITE (nlog,"(' Calling radhyd_to_edit, &
...
IF ( myid >= ml  .and.  myid <= mu ) WRITE (nlog,"(' Calling radhyd_to_monitor, &
& ij_ray_dim, ik_ray_dim, nx, ny, nz, nez, nnu, nnc=',8i4)") &
...
!-----------------------------------------------------------------------
  ...
  WRITE (nlog,"(' Calling radhyd_x_MPI_W, nx, ny, nz, nez, nnu, nnc, ndim, &
&  ij_ray_min, ij_ray_max, ij_ray_dim, ik_ray_min, ik_ray_max, ik_ray_dim, &
...
&  ik_ray_min, ik_ray_max, ik_ray_dim, i_edit, time, dtnph, nu_equil
  ...
  CALL radhyd_x_MPI_W( nx, ny, nz, nez, nnu, nnc, ndim, ij_ray_min, &
...
!-----------------------------------------------------------------------
    ...
    WRITE (nlog,"(' Calling radhyd_y_MPI_W, jmin, jmax, nx, ny, nz, nez, nnu, &
...
&    j_ray_dim, n_proc, n_proc_y, n_proc_z, dtnph, nu_equil, sub_cy_yz
  ...
  IF ( ndim > 1  .and.  ny > 1 ) THEN
    CALL radhyd_y_MPI_W( jmin, jmax, nx, ny, nz, nez, nnu, nnc, ndim, &
...
!-----------------------------------------------------------------------
    ...
    WRITE (nlog,"(' Calling radhyd_z_MPI_W, kmin, kmax, nx, ny, nz, nez, nnu, &
...
&    k_ray_dim, n_proc, n_proc_y, n_proc_z, dtnph, nu_equil, sub_cy_yz
  ...
  IF ( ndim > 1  .and.  nz > 1 ) THEN
    CALL radhyd_z_MPI_W( kmin, kmax, nx, ny, nz, nez, nnu, nnc, ndim, &
...
!-----------------------------------------------------------------------
  ...
  WRITE (nlog,"(' Calling radyhd_to_time_step_select_MPI, nx, ny, nz, &
...
!-----------------------------------------------------------------------
  ...
  CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
  WRITE (nlog,"(' Calling radhyd_to_restart, ndim, nx, nez, nnu, myid=',5i4)") &
&  ndim, nx, nez, nnu, myid
...
!-----------------------------------------------------------------------
  ...
  WRITE (nlog,"(' Calling radhyd_x_MPI, nx, ny, nz, nez, nnu, nnc, ndim, &
&  ij_ray_min, ij_ray_max, ij_ray_dim, ik_ray_min, ik_ray_max, ik_ray_dim, &
&  i_edit, time, dtnph, nu_equil=',14i4, 2es11.3, a2)") &
&  nx, ny, nz, nez, nnu, nnc, ndim, ij_ray_min, ij_ray_max, ij_ray_dim, &
&  ik_ray_min, ik_ray_max, ik_ray_dim, i_edit, time, dtnph, nu_equil
