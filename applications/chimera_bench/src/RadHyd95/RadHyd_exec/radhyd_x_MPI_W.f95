SUBROUTINE radhyd_x_MPI_W( nx, ny, nz, nez, nnu, nnc, ndim, ij_ray_min, &
& ij_ray_max, ij_ray_dim, ik_ray_min, ik_ray_max, ik_ray_dim, i_edit, &
& time, dtnph, nu_equil )
!-----------------------------------------------------------------------
!
!    File:         radhyd_x_MPI_W
!    Module:       radhyd_x_MPI_W
!    Type:         Program
...
!  set_final_radial_grid_MPI  : set the final radial grid
...
!    Include files:
!  kind_module
!  bomb_module, edit_module, evh1_sweep, parallel_module,
!  prb_cntl_module
!
!-----------------------------------------------------------------------
...
IF ( myid >= ml  .and.  myid <= mu ) WRITE (nlog,*) ' Calling store_int_grid'
...
CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
IF ( myid >= ml  .and.  myid <= mu ) WRITE (nlog,*) ' Calling store_int_radial_var'
...
CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
IF ( myid >= ml  .and.  myid <= mu ) &
& WRITE (nlog,"(' Calling radhyd_to_monitor_MPI, ij_ray_dim, &
&  ik_ray_dim, nx, ny, nz, nez, nnu, nnc=',8i4)") &
&  ij_ray_dim, ik_ray_dim, nx, ny, nz, nez, nnu, nnc

CALL radhyd_to_monitor_MPI( ij_ray_dim, ik_ray_dim, nx, ny, nz, nez, &
& nnu, nnc )
...
CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
IF ( myid >= ml  .and.  myid <= mu )  &
& WRITE (nlog,"(' Calling set_final_radial_grid_MPI, ij_ray_dim, ik_ray_dim, &
&  nx=',3i4)") &
&  ij_ray_dim, ik_ray_dim, nx

CALL set_final_radial_grid_MPI( ij_ray_dim, ik_ray_dim, nx )
