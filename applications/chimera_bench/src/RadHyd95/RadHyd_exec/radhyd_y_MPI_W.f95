SUBROUTINE radhyd_y_MPI_W( jmin, jmax, nx, ny, nz, nez, nnu, nnc, ndim, &
& ij_ray_dim, ik_ray_dim, j_ray_dim, n_proc, n_proc_y, n_proc_z, dtnph, &
& nu_equil, sub_cy_yz )
!-----------------------------------------------------------------------
!
!    File:         radhyd_y_MPI_W
!    Module:       radhyd_y_MPI_W
!    Type:         Subprogram
...
IF ( myid >= ml  .and.  myid <= mu  .and.  print_y ) &
...
&      nx, ny, ji_ray, jk_ray, j_ray_dim, ik_ray_dim, i_radial, nez, nnu, nnc
...
!        CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
        IF ( myid >= ml  .and.  myid <= mu  .and.  print_y ) &
&        WRITE (nlog,"(' Calling radhyd_to_equilibrate_y, nx, ny, nez, nnu, &
&        ji_ray, jk_ray, j_ray_dim, ik_ray_dim, i_radial=',9i4)") &
