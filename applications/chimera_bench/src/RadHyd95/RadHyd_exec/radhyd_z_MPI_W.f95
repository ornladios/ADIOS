SUBROUTINE radhyd_z_MPI_W( kmin, kmax, nx, ny, nz, nez, nnu, nnc, ndim, &
& ij_ray_dim, ik_ray_dim, k_ray_dim, n_proc, n_proc_y, n_proc_z, dtnph, &
& nu_equil, sub_cy_yz )
!-----------------------------------------------------------------------
!
!    File:         radhyd_z_MPI_W
!    Module:       radhyd_z_MPI_W
!    Type:         Subprogram
...
USE parallel_module, ONLY : myid, myid_z, ierr
...
INTEGER, PARAMETER               :: ml = 0          ! lower myid to print
INTEGER, PARAMETER               :: mu = 10000      ! upper myid to print
...
IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
& WRITE (nlog,"(' Calling radhyd_to_transpose_x_z,  nx, ij_ray_dim, &
&  ik_ray_dim, nz, k_ray_dim, ny, nez, nnu, nnc, n_proc, n_proc_y, &
&  n_proc_z=',12i4)") &
&  nx, ij_ray_dim, ik_ray_dim, nz, k_ray_dim, ny, nez, nnu, nnc, n_proc, &
&  n_proc_y, n_proc_z
...
WRITE (nlog,"(' Calling set_final_azimuthal_grid')") 
...
!-----------------------------------------------------------------------
    ...
    CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
    IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&   WRITE (nlog,"(' Calling radhyd_to_eos_z_reset 1, nz, ki_ray, &
&    kj_ray, ij_ray_dim, k_ray_dim, nnc=',6i4)") &
&    nz, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, nnc
...
!-----------------------------------------------------------------------
    ...
    CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
    IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&   WRITE (nlog,"(' Calling radhyd_to_hydro_z_time_step, kmin, kmax, &
&    ki_ray, kj_ray, ij_ray_dim, k_ray_dim, dt_z_hydro=',6i4,es11.3)") &
&    kmin, kmax, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, dt_z_hydro
...
!      CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
      IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&     WRITE (nlog,"(' Calling store_int_azimuthal_var, ki_ray, kj_ray, nz',3i4)") &
&      ki_ray, kj_ray, nz
...
!      CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
      IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&     WRITE (nlog,"(' Calling radhyd_to_nu_stress_z, ki_ray, kj_ray, &
&      ij_ray_dim, k_ray_dim, i_radial, j_radial, nx, ny, nz, nez, nnu=',11i4)") &
&      ki_ray, kj_ray, ij_ray_dim, k_ray_dim, i_radial, j_radial, nx, ny, &
&      nz, nez, nnu
...
!      CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
      IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&     WRITE (nlog,"(' Calling radhyd_to_evh1_z_lagr, nx, ny, nz, ki_ray, &
&      kj_ray, ij_ray_dim, k_ray_dim, i_radial, nez, nnu=',10i4)") &
&      nx, ny, nz, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, i_radial, nez, nnu
...
!      CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
      IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&     WRITE (nlog,"(' Calling radhyd_to_eos_z_reset 2, nz, ki_ray, &
&      kj_ray, ij_ray_dim, k_ray_dim, nnc=',6i4)") &
&      nz, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, nnc
...
!      CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
      IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&     WRITE (nlog,"(' Calling radhyd_to_nu_e_advct_z, nx, nz, ki_ray, &
&      kj_ray, ij_ray_dim, k_ray_dim, i_radial, j_radial, nez, nnu=',10i4)") &
&      nx, nz, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, i_radial, j_radial, nez, nnu
...
!      CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
      IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&     WRITE (nlog,"(' Calling radhyd_to_remap_z, nx, nz, ki_ray, kj_ray, &
&      ij_ray_dim, k_ray_dim, i_radial, nez, nnu, nnc=',10i4)") &
&      nx, nz, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, i_radial, nez, nnu, nnc
...
!      CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
      IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&     WRITE (nlog,"(' Calling radhyd_to_eos_z_reset 3, nz, ki_ray, kj_ray, &
&      ij_ray_dim, k_ray_dim, nnc=',6i4)") &
&      nz, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, nnc
...
!        CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
        IF ( myid >= ml  .and.  myid <= mu  .and.  print_z ) &
&       WRITE (nlog,"(' Calling radhyd_to_equilibrate_z nx, ny, nez, &
&       nnu, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, i_radial=',9i4)") &
&        nx, nz, nez, nnu, ki_ray, kj_ray, ij_ray_dim, k_ray_dim, i_radial
...
CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
IF ( myid >= ml  .and.  myid <= mu ) &
& WRITE (nlog,"(' Calling radhyd_to_transpose_z_x, nx, ij_ray_dim, &
&  ik_ray_dim, nz, k_ray_dim, nez, nnu, nnc, n_proc, n_proc_z=',10i4)") &
&  nx, ij_ray_dim, ik_ray_dim, nz, k_ray_dim, nez, nnu, nnc, n_proc, &
&  n_proc_z
