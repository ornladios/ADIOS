...
!  cycle_module, edit_module, parallel_module
!
!-----------------------------------------------------------------------
...
INTEGER, DIMENSION(2,ij_ray_dim,ik_ray_dim)                   :: isend_buf ! send buffer for gathering data from processors
INTEGER, DIMENSION(2,ij_ray_dim,ik_ray_dim,n_proc_y*n_proc_z) :: irecv_buf ! receive buffer for gathering data from processors
...
!-----------------------------------------------------------------------
  ...
  101 FORMAT (' Cannot find i_out in subroutine regrid_final_grid_MPI; rhobar0(1)=',es11.3, &
&  ' rhobar0(imax)=',es11.3,' rho_min=',es11.3,' run stopped')
  ...
  111 FORMAT (' Iteration will not converge for rho_test in subroutine regrid_final_grid_MPI for determining zone ',i4,/ &
&      ' dvolume_test=',es11.3,' dvol_0(nem1)=',es11.3,' rho_test=',es11.3, &
&      ' rhobar_rgrd(i)=',es11.3)
  113 FORMAT (' Iteration will not converge for rho_test in subroutine regrid_final_grid_MPI for determining zone ',i4,/ &
&      ' for the case in which i and i-1 are in the same original zone',/ &
&      ' dvolume_test=',es11.3,' dvol_0(nem1)=',es11.3,' rho_test=',es11.3, &
&      ' rhobar_rgrd(i)=',es11.3,' run stopped')
  115 FORMAT (' Iteration will not converge for rho_test in subroutine regrid_final_grid_MPI for determining zone ',i4,/ &
&      ' for the case in which i and i-1 are in the same original zone',/ &
&      ' dvolume_test=',es11.3,' dvolume_i_nem1=',es11.3,' rho_test=',es11.3, &
&      ' rhobar_rgrd(i)=',es11.3)
  121 FORMAT (' Grid adjusted; i_out=',i4)
  123 FORMAT (' Regridding is being attempted in regrid_final_grid_MPI from rho_max=',es11.3, &
...
isend_buf(1,:,:)         = jjshockmn(:,:)
isend_buf(2,:,:)         = inse_mn(:,:)

!-----------------------------------------------------------------------
!  Gather the jjshockmn's and the inse_mn's
!-----------------------------------------------------------------------

c_gath_recv              = 2 * ij_ray_dim * ik_ray_dim
c_gath_send              = 2 * ij_ray_dim * ik_ray_dim

CALL MPI_Barrier( MPI_COMM_WORLD, ierr )
CALL MPI_GATHER( isend_buf, c_gath_send, MPI_INTEGER, irecv_buf, c_gath_recv, &
...
IF ( myid == 0 ) THEN
  ...
  DO mm = 0,n_proc-1
    ...
    DO k = 1,ik_ray_dim
      ...
      DO j = 1,ij_ray_dim
        jsk              = mj * ij_ray_dim + j
        jjshockmn_a(jsk,ksk) = irecv_buf(1,j,k,mm+1)
        inse_mn_a(jsk,ksk)   = irecv_buf(2,j,k,mm+1)
      END DO ! j = 1,ij_ray_dim
    END DO ! k = 1,ik_ray_dim
  END DO ! mm = 0,n_proc-1
...
i_extent                 = 1
CALL MPI_BCAST( i_out, i_extent, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
