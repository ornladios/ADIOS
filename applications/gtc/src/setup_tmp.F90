...
!! initialize restart directory and control var
  ...
  restart_dir1="databack1"
  restart_dir2="databack2"
...
!$omp parallel do private(i,j,k,r,ij,zdum,tdum,rmax,rmin)
  ...
  if(track_particles == 1)then
    ...
    ! the data to file every (mstep/msnap) steps.
     ...
     nspec=1
     if(nhybrid>0)then
         nspec=2    !!two species 1: ion, 2: electron
     endif
     allocate(ptracked(nparam,max(mimax,memax,1),nspec)) !!array to store tracked particle information
     allocate(ntrackp(nspec))  !! actual tracked particle number in a processor
     ptracked=0.0
     ntrackp=0
  else
...
!  write(0,*)mype,' in net_particle_decomp: mzeta=',mzeta,'  zetamin=',&
