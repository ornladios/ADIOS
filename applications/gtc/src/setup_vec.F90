...
!========================================================================
  ...
  integer,allocatable,dimension(:,:) :: pe_grid
  integer,allocatable,dimension(:) :: toroidal_domain_location,&
...
#ifdef _SX
! ------ Trick to minimize bank conflicts on the ES/SX-6 -----------
!
! First, let's make sure that maxmpsi is greater than mpsi. If not, it
! needs to be changed in the module file.
  if (mpsi > maxmpsi) then
     write(0,*)'mpsi=',mpsi,'  >  maxmpsi=',maxmpsi
     call MPI_ABORT(MPI_COMM_WORLD,1,ierror)
  endif

  mmtheta(0:mpsi) = mtheta
!cdir du_update(mmtheta)
  qqtinv(0:mpsi) = qtinv
!cdir du_update(qqtinv)
! ------------------------------------------------------------------
#endif
...
! In the case of a non-linear run, we need only half the points.
  if(nonlinear > 0.5)mtdiag=mtdiag/2
  if(mype.eq.0)write(0,*)'mtdiag final =',mtdiag
...
! allocate memory
  allocate(pgyro(5,mgrid),tgyro(5,mgrid),markeri(mzeta,mgrid),&
...
! allocate memory
  allocate(zion(nparam,mimax),zion0(nparam,mimax),jtion0(mimax,4),&
     jtion1(mimax,4),kzion(mimax),wzion(mimax),wpion(mimax,4),&
     wtion0(mimax,4),wtion1(mimax,4),STAT=mtest)
  if (mtest /= 0) then
...
! dtheta/delta_x=1/(r*(1+r*cos(theta))), delta_x=poloidal length increasement
...
! number since this quantity will be used in Fast Fourier Transforms
  ...
  allocate(pe_grid(0:npartdom-1,0:ntoroidal-1),&
          &toroidal_domain_location(0:numberpe-1),&
          &particle_domain_location(0:numberpe-1),STAT=mtest)
  if (mtest /= 0) then
     write(0,*)mype,'*** Cannot allocate qtinv: mtest=',mtest
     call MPI_ABORT(MPI_COMM_WORLD,1,ierror)
  endif
...
! particle and toroidal domain numbers. Given these 2 numbers, the 2D array
! "pe_grid(i,j)" will return the id number of the processor holding that
! domain. Inversely, given the PE number, particle_domain_location(PE_number)
! and toroidal_domain_location(PE_number) will return the 2 numbers
! identifying the domain where PE_number sits.
! 
...
! Test the consistency of the numbers
  if(pe_number/=numberpe)then
    write(0,*)'**** IN set_particle_decomp: pe_number NOT EQUAL to numberpe!!'
    call MPI_ABORT(MPI_COMM_WORLD,1,ierror)
  endif
  i=particle_domain_location(mype)
  j=toroidal_domain_location(mype)
  if(pe_grid(i,j)/=mype)then
    write(0,*)'**** IN set_particle_decomp: pe_number NOT EQUAL to mype=',mype
    call MPI_ABORT(MPI_COMM_WORLD,1,ierror)
  endif

!! Save coordinates of left and right toroidal neighbors of current process.
!! We define the toroidal neighbors as pe_grid(i,j+1) and pe_grid(i,j-1)
!! with periodic boundary conditions for j=1 and j=ntoroidal
!! This is for communications via MPI_COMM_WORLD. We will instead create
!! a new toroidal communicator that will link all the PEs with the same
!! particle domain and use that communicator instead.
!  jm1=mod((j-1)+ntoroidal,ntoroidal)
!  left_neighbor=pe_grid(i,j-1)
!  right_neighbor=pe_grid(i,j+1)


!  write(0,*)'mype=',mype,"  particle_domain_location =",&
!            particle_domain_location(mype),' toroidal_domain_location =',&
!            toroidal_domain_location(mype),' pi=',pi

! Domain decomposition in toroidal direction.
  mzeta=mzetamax/ntoroidal
  zetamin=2.0_wp*pi*real(toroidal_domain_location(mype))/real(ntoroidal)
  zetamax=2.0_wp*pi*real(toroidal_domain_location(mype)+1)/real(ntoroidal)
...
! Each one of these processes will do a charge deposit on a copy of
...
!  call MPI_COMM_SPLIT(old_comm,color,rank_in_new_comm,new_comm,ierror)
  ...
  i=particle_domain_location(mype)
  j=toroidal_domain_location(mype)
  call MPI_COMM_SPLIT(MPI_COMM_WORLD,j,i,partd_comm,ierror)
  call MPI_COMM_SPLIT(MPI_COMM_WORLD,i,j,toroidal_comm,ierror)
