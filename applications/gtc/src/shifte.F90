...
! an extra space to prevent zero size when msendright(1)=msendleft(1)=0
  allocate(sendright(nzelectron,max(1,msendright(1))),sendleft(nzelectron,max(1,msendleft(1))))

! pack particle to move right
  !$omp parallel do private(m)
     do m=1,msendright(1)
        sendright(1:nparam,m)=zelectron(1:nparam,iright(m))
        sendright(nparam+1:nzelectron,m)=zelectron0(1:nparam,iright(m))
     enddo

! pack particle to move left
!$omp parallel do private(m)
     do m=1,msendleft(1)    
        sendleft(1:nparam,m)=zelectron(1:nparam,ileft(m))
        sendleft(nparam+1:nzelectron,m)=zelectron0(1:nparam,ileft(m))
     enddo
...
! fill the hole
     ...
     do i=1,msend
        ...
        zelectron(1:nparam,m)=zelectron(1:nparam,mtop)
        zelectron0(1:nparam,m)=zelectron0(1:nparam,mtop)
        mtop=mtop-1
...
! send # of particle to move right
  ...
  allocate(recvleft(nzelectron,max(1,mrecvleft(1))))
...
! send # of particle to move left
  ...
  allocate(recvright(nzelectron,max(1,mrecvright(1))))
...
!$omp parallel do private(m)
  do m=1,mrecvleft(1)
     zelectron(1:nparam,m+me)=recvleft(1:nparam,m)
     zelectron0(1:nparam,m+me)=recvleft(nparam+1:nzelectron,m)
  enddo

! particle moved from right
!$omp parallel do private(m)
  do m=1,mrecvright(1)
     zelectron(1:nparam,m+me+mrecvleft(1))=recvright(1:nparam,m)
     zelectron0(1:nparam,m+me+mrecvleft(1))=recvright(nparam+1:nzelectron,m)
  enddo
