...
!========================================
  ...
  implicit none
  integer :: i,m,np,npp
  real(wp) :: r,ainside,aoutside,acentre,awidth
...
!  if(iout==0)iout=isnap
  ...
  do m=1,mi
     if(zion(7,m)>0.0)then
       npp=npp+1
       ptracked(1:nparam,npp,1)=zion(1:nparam,m)
     endif
  enddo
  ntrackp(1)=npp
  npp=0
  ...
  if (nhybrid>0) then
     do m=1,me
        if(zelectron(7,m)>0.0)then
          npp=npp+1
          ptracked(1:nparam,npp,2)=zelectron(1:nparam,m)
        endif
     enddo
  endif
  ntrackp(2)=npp
...
!========================================
  ...
  character(len=10) :: cdum
...
#if ADIOS
    comm=MPI_COMM_WORLD
    write(fname,'("TRACKP_",i5.5,".bp")')(1+(mstepall+istep-ndiag)/isnap)
    if(mype==0)write(*,*)"tracking filename",fname
    fname=trim(fname)//char(0)
    write(dirstr,'("node_",i5.5,"_t_",i5.5)')mype,(mstepall+istep)/ndiag
    ...
    call adios_write(buf_id,'ptrackedi'//char(0),ptracked(:,1:ntrackp(1),1))
    if(nhybrid>0)then
        call adios_write(buf_id,'ntracke'//char(0),ntrackp(2))
        call adios_write(buf_id,'ptrackede'//char(0),ptracked(:,1:ntrackp(2),2))
    endif
