...
!!!        ADIOS_WRITE(_handle,toroidal_comm)
!!!        ADIOS_WRITE(_handle,mpsi+1)
!!!        ADIOS_WRITE(_handle,mzeta+kp)
!!!!write data
!!!        ADIOS_WRITE(_handle,myrank_toroidal)
!!!        ADIOS_WRITE(_handle,mpsi)
!!!        ADIOS_WRITE(_handle,mzeta)
!!!        ADIOS_WRITE(_handle,kp)
!!!        ADIOS_WRITE(_handle,nproc_toroidal)

!!! added by zf2
!!!        if(myrank_toroidal==0) then 
!!!          ADIOS_WRITE(_handle,radial)
!!!          call adios_write(_handle,"mtheta"//char(0),(mtheta+1), adios_err)
!!!          ADIOS_WRITE(_handle,itran)
!!!        endif
      !!!   call adios_write(_handle,"mpsi_dim"//char(0),mpsi)

!!! added by zf2
!!!     ADIOS_WRITE(_handle,(mzeta+kp)*nproc_toroidal)
!!!     ADIOS_WRITE(_handle,(mzeta+kp)*myrank_toroidal)

      !!!   ADIOS_WRITE(_handle,theta_start)

!!!        ADIOS_WRITE(_handle,mgrid)

!!! added by zf2
!!!        ADIOS_WRITE(_handle,mgrid*nproc_toroidal)
!!!        ADIOS_WRITE(_handle,mgrid*myrank_toroidal)
    
!!!        ADIOS_WRITE(_handle,X)
!!!        ADIOS_WRITE(_handle,Y)
!!!        ADIOS_WRITE(_handle,Z)
...
!!!  call adios_write_offset (_handle, phi_offset)
!!!  ADIOS_WRITE(_handle,toroidal_comm)
!!!  ADIOS_WRITE(_handle,mzeta+kp)  ! TODO check this for correctness
...
!!!    call adios_write(_handle,"mtheta"//char(0),(mtheta+1),adios_err)
!!!  endif
!!!  call adios_write (_handle, "phi"//char(0), dataout,adios_err)
