...
!         print *,'total mstep=',mstep
  ...
  101 format(i6)
     ...
     if(mype==0) then
        open(666,file='history.out',status='old')
        do i=1,4
           read(666,101)j
        enddo
        ...
        print *,'mstepall for this run=',mstepall
     endif
     call mpi_bcast(mstepall,1,mpi_integer,0,mpi_comm_world,ierr)
     call MPI_BARRIER(MPI_COMM_WORLD,ierr)
