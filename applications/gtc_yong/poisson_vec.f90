...
! Vectorized version of the above loop. We explicitly set ring(j,i,k)=0
! for j=nindex(i,k)+1,mindex in subroutine poisson_initial.
!$omp parallel do private(i)
        do i=1,mgrid
           ptilde(i)=0.0
        enddo
...
! This next loop is to fix the vectorization problem. We explicitly set
! to zero all the unused parts of the ring() array so that a loop from
! 1 to nindex(i,k) can be extended to mindex.
  do k=1,mzeta
     do i=1,mgrid
        do j=nindex(i,k)+1,mindex
           ring(j,i,k)=0.0
           indexp(j,i,k)=j-nindex(i,k)
        enddo
     enddo
  enddo
  ...
  if(mype==0)then
     ...
     write(stdout,*)'poisson solver=',maxval(nindex),minval(nindex),&
