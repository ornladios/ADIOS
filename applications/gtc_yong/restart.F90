...
!!!     call adios_set_path (group_id,dirstr//char(0));
     ...
     if (iop=="read") then
        ...
        !!!call adios_open_read (adios_handle, group_id, restart_fname)
        call adios_open (adios_handle, 'restart'//char(0), restart_fname, 'r'//char(0), adios_err)
     else
        !!! modified by zf2 for new ADIOS API
        !!!call adios_open (adios_handle, group_id, restart_fname)
...
!     call adios_get_data_size (adios_handle, mype_filesize)
     call adios_close (adios_handle,adios_err)
