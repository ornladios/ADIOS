...
!     start_time = MPI_WTIME()
  ...
  if(iop=="write")then
     ...
     call adios_write(buf_id,"zion0"//char(0),zion0(6,1:mi))
     ...
     if(nhybrid>0)then
        ...
        call adios_write(buf_id,"zelectron0"//char(0),zelectron0(6,1:me))
           if(mype==0)then
              if(stdout /= 6 .and. stdout /= 0)open(stdout,file='stdout.out',status='old',position='append')
                  write(stdout,*)'diagnosis finish'
              if(stdout /= 6 .and. stdout /= 0)close(stdout)
           endif
...
!     if(myrank_partd<nproc_partd-1)call MPI_Wait(mrequest,mstatus,merror)
  ...
  if(mype==0)write(stdout,*)"READ FILE CLOSED"
#endif
