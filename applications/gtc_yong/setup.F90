  ...
  if(track_particles == 1)then
    ...
    ! the data to file every (mstep/msnap) steps.
     ...
     if(nhybrid>0)then
         ...
         allocate(ptrackede(nparam,max(memax,1)))  !!array to store electron tracked particles
     endif
     ptrackedi=0.0
     ptrackede=0.0
     ntrackp=0
...
!XY intitialize
  ...
  if(nhybrid>0)then
     allocate(zelectron(nparam,memax),zelectron0(nparam,memax),jtelectron0(memax),&
        jtelectron1(memax),kzelectron(memax),wzelectron(memax),&
        wpelectron(memax),wtelectron0(memax),wtelectron1(memax),&
        markere(mzeta,mgrid),densitye(0:mzeta,mgrid),zelectron1(nparam,memax),&
