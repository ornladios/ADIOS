...
!!XY rolling restart
  allocate(phip00Ntmp(0:mpsi),pfluxpsiNtmp(0:mpsi),zonaliNtmp(0:mpsi),zonaleNtmp(0:mpsi),&
     rdtemiNtmp(0:mpsi),rdtemeNtmp(0:mpsi),STAT=mtest)
...
!!XY rolling restart
  allocate(phiNtmp(0:mzeta,mgrid),STAT=mtest)
...
!!XY rolling restart
  allocate(zionNtmp(nparam,mimax),zion0Ntmp(nparam,mimax),,STAT=mtest)
...
!!XY rolling restart
     allocate(zelectronNtmp(6,memax),zelectron0Ntmp(6,memax),&
        phisaveNtmp(0:mzeta,mgrid,2*nhybrid),STAT=mtest)
