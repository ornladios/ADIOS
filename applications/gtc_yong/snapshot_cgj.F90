subroutine snapshot
  ...
  integer i,ibin,ip,iu,j,jj,jm,jt,k,kz,m,nsnap,ierror,icount,file_hd,startidx,endidx
  ...
  real(wp),dimension(0:mpsi) :: marker,fflows,dflows,ftem,dtem,adum
  real(wp),dimension(mbin_u,mbin_psi) :: ubin,dubin,pbin,dpbin,ebin,debin,bdum
...
! number of poloidal grid
  jm=mtheta(mpsi/2)
  !if(mype==0)write(stdout,*)"jm,mpsi,mbin_u",jm,mpsi,mbin_u
...
! number of quantities
     do i=1,mbin_u
        dataout_u(i,1)=uleft+(uright-uleft)*real(i-1)/real(mbin_u-1)
        dataout_u(i,1)=-1.0+2.0*real(i-1)/real(mbin_u-1)
        dataout_u(i,1)=1.0/eright*real(i-1)/real(mbin_u-1)
     enddo
! velocity space distribution function
     ...
     do j=0,jm
        ...
        do i=1,mpsi
           ...
           booz_out(i,j,1)=boozer2x(pdum,tdum)
           booz_out(i,j,1)=boozer2z(pdum,tdum)
           booz_out(i,j,3)=(wt*phi(0,igrid(i)+jt)+(1.0-wt)*phi(0,igrid(i)+jt-1))/&
                gyroradius**2
        enddo
     enddo
     do j=1,jm
        do kz=1,ntoroidal
           do k=1,mzeta
              flux3darray(k,kz,j)=eachpsi((kz-1)*mzeta*jm+(j-1)*mzeta+k)/gyroradius**2
           enddo
        enddo
     enddo
     do j=1,mpsi
        dataout_r(j,1)=a0+deltar*(real(j))/gyroradius
     enddo
...
! record program end time
  if(istep .eq. mstep)then
     ...
     if(mype == 0)write(stdout,*) 'Program ends at DATE=', date, 'TIME=', time
  endif
101 format(i6)
102 format(e10.4)
