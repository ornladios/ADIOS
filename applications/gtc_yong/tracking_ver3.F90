...
!  
  ...
  if(mype==0)write(*,*)'ainside=',ainside,'aoutside=',aoutside
!!XY ion information temperarily removed
! np=0
! do m=1,mi
!    
...
!! enddo
  ...
  if(mype==0)write(*,*)'me=',me,'np=',np,'zelectron(1,1:6)=',zelectron(7,1:6),&
...
!!  enddo
  ...
  ntrackp(2)=npp
  if(mype==0)write(*,*)'mype=0: npp=',npp
...
#if ADIOS
    comm=MPI_COMM_WORLD
    write(fname,'("TRACKP_",i5.5,".bp")')mstepall+istep  !!(1+(mstepall+istep-ndiag)/isnap)
    if(mype==0)write(*,*)"tracking filename ",fname
    fname=trim(fname)//char(0)
    write(dirstr,'("node_",i5.5,"_t_",i5.5)')mype,mstepall+istep !!!(mstepall+istep)/ndiag
    dirstr=trim(dirstr)//char(0)
    call adios_get_group (group_id, "particles"//char(0))
    call adios_set_path (group_id,dirstr//char(0));
    call adios_open_append(buf_id, group_id, fname)
...
!!    call adios_write(buf_id,'ptrackedi'//char(0),ptrackedi(:,1:ntrackp(1)))
    ...
    if(mype==0)write(*,*)'mype=0:ntrackp(2)=',ntrackp(2)
    if(mype==0)write(*,*)'me=',me,'ntrackp(2)=',ntrackp(2),'zelectron(1,1:6)=',ptrackede(7,1:6),&
                        'zelectron(1:8,1)= ',ptrackede(1:8,1)
