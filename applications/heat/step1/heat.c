...
void update(int start, int end,float *u,float *u2);
...
/* First, find out my taskid and how many tasks are running */
   ...
   if (taskid == MASTER)
   {
	...
	sprintf(resp,"%s%d%s","data",it,".dat"); 
	if (it%STRD==0)  prtdat(&u[0][0][0], resp);
      }
  ...
  for (ix = 0; ix <= NX-1; ix++) 
  {
    for (iy = 0; iy <=NY-1; iy++) 
    {
      if( ( ((iy-1)%6)==0 &&iy>=7) || iy==0)
       ...
       fprintf(fp, " %8.6f", ix,iy,*(u+ix*NY+iy));
       if (iy == NY-1) 
           fprintf(fp, "\n");
    }
  }
