...
void prtdat(float *u1, char *fname); 
...
/* First, find out my taskid and how many tasks are running */
   ...
   if (taskid == MASTER)
   {
      ...
      printf("Initializing grid and writing initial.bpfile...\n");
      inidat((float *) u,1.0);
      prtdat((float *) u, "initial.bp");
	...
	if (it%STRD==0)  prtdat(&u[0][0][0], filename);
      }
 ...
 * subroutine prtdat
 **************************************************************************/
void prtdat(float *u, char *fname) 
{
   /* Add Variables*/
   float cos_u[NX][NY];
   float sin_u[NX][NY];
   ...
   // assign the dataset to buffer
   adios_write (buf, "data", u);
   adios_write(buf,"cos_data",cos_u);
   adios_write(buf,"sin_data",sin_u);
   adios_write(buf,"tan_data",tan_u);
