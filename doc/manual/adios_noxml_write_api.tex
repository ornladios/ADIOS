...
\verb+adios_stat_no+,  \verb+adios_stat_default+, or \verb+adios_stat_full+. If stats is set to \verb+adios_stat_default+, 
ADIOS internally calculates and outputs min and max for each processor automatically. If it is set to full, additional statistics are calculated including partially calculated data for average and standard deviation, which are completed at read time. 
