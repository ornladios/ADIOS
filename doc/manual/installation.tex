...
Note: There is a cmake\_init batch script in the trunk set up for our machines. Studying
it can help you setting up the appropriate environment variables and configure options for your system.
...
the "build" folder, a completely separate directory, which will make source tree without cluttering up. To do this,
users first create a build directory with "mkdir build" in your top-level folder 
and then switch to the build directory and run cmake for ADIOS there.

If crossing-compiling is required for a certain compiler, for example the intel compiler on Titan, 
...
to
\begin{lstlisting}
cmake -DCMAKE_TOOLCHAIN_FILE=../toolchain_intrepid.cmake ..
\end{lstlisting}
...
\noindent or in CMake
\begin{lstlisting}
export CRAY_UGNI_DIR=<Point to the UGNI installation>
export CRAY_PMI_DIR=<Point to the PMI installation>
\end{lstlisting}
...
\noindent or in CMake
\begin{lstlisting}
export DATASPACES_DIR=<Point to the DATASPACES installation>
\end{lstlisting}
