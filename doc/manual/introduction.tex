...
\noindent The big news in version 1.6 is that on-the-fly {\bf data transformations} is supported in file-based I/O. The following transformations are supported:
\begin{itemize}
\item lossless compression (zlib, bzip, szip, ALACRITY)
\item lossy compression (ISOBAR)
\item APLOD indexing + lossless compression
\end{itemize}

One can select a transformation of \emph{each variable} separately in the XML configuration file, so no recompilation of the code is needed. ALACRITY, ISOBAR and APLOD are not public software at this moment, but they are available for research. To get them, send a request to \verb+"Nagiza Samatova" <samatova@csc.ncsu.edu>+.

\vspace{10pt}

\noindent The C version of the READ API is extended with functions to get information about the {\bf visualization schema} stored in the dataset. The file structure returned by \verb+adios_open()+ contains the name list of meshes defined in the dataset. \verb+adios_inq_mesh_byid()+ returns a structure describing a mesh, and \verb+adios_inq_var_meshinfo()+ tells on which mesh should one visualize a given variable. 
