...
//	ADIOS_FILE * f = adios_fopen ("adios_global.bp", comm);
	...
	ADIOS_FILE * f = adios_fopen (bp_file_name, comm);
    if (f == NULL)
    {
        printf ("%s\n", adios_errmsg());
        return -1;
	...
	for(i = 0; i < slice_size * v->dims[1]; i++)
	{
		printf(" %6.6g", ((double *)data)[i]);
	}
	printf("\n");
