...
int main (int argc, char ** argv) 
{
    ...
    enum ADIOS_READ_METHOD method = ADIOS_READ_METHOD_ICEE;
    ...
    sprintf(initstring, "cm_port=%d", cm_port + rank);
    
    adios_read_init_method(method, comm, initstring);
    ...
    while(ierr >= 0) {
        ...
        uint64_t xcount = nx_val;
        uint64_t ycount = ny_val;
        //uint64_t xcount = arry->dims[0];
        //uint64_t ycount = arry->dims[1];
    
        uint64_t starts[] = {0,rank * ny_val};
        ...
        ierr = adios_advance_step(afile, 0, 30);
