...
//CJ, numeric attribute specified as scalar 
            ...
            printf("cj attr:%s %s %d", value,path,t1);         
...
//CJ
    ...
    else if(attr->type==adios_integer)
    {
       attr->value=adios_find_var_by_name (g->vars, value)->data;
    }
    else
    {
        fprintf (stderr, "unspported attribute type: %d\n", attr->type);
	return 0;
    }
    attr->next = 0;
                             ...
                             )
{
    ...
    else if(a->type==adios_integer)
    {
         //if(a->name[strlen(a->name)-2]=="/")
            bw_attr_num_gp(buf, start, &end, a->path, a->name, a->value,a->type);
         //else
         //   bw_attr_num_ds(buf, start, &end, a->path, a->name, a->value,a->type);
    }
    buf_start = start;
    *buf_end = end;
...
//CJ
    if(a->type==adios_string)
       ...
       size = bcalsize_attr_num (a->path, a->name, a->type,a->value);
