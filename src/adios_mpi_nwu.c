...
static int adios_mpi_nwu_initialized = 0;
...
struct adios_MPI_data_struct
{
    ...
    int striping_unit;  /* file system stripe size */
};
...
static int
adios_mpi_nwu_get_striping_unit(MPI_File fh, char *filename)
{
    struct statfs fsbuf;
    int err, flag, striping_unit = 1048576;
    char     value[64];
    MPI_Info info_used;
...
static int
adios_mpi_nwu_striping_unit_write(MPI_File    fh,
                              MPI_Offset  offset,
                              void       *buf,
                              int         len,
                              int         striping_unit)
{
    ...
    if (striping_unit > 0) {
        ...
        err = 0;
        while (rem_size > 0) {
            int rem_unit  = striping_unit - rem_off % striping_unit;
            int write_len = (rem_unit < rem_size) ? rem_unit : rem_size;
            int ret_len;

#ifdef _WKL_CHECK_STRIPE_IO
printf("adios_mpi_nwu_striping_unit_write offset=%12lld len=%12d\n",offset,write_len);offset+=write_len;
#endif
...
void adios_mpi_nwu_init (const char * parameters
                    ,struct adios_method_struct * method
                    )
{
    struct adios_MPI_data_struct * md = (struct adios_MPI_data_struct *)
                                                    method->method_data;
    if (!adios_mpi_nwu_initialized)
    {
        adios_mpi_nwu_initialized = 1;
    }
...
int adios_mpi_nwu_open (struct adios_file_struct * fd
                   ,struct adios_method_struct * method
                   )
{
    struct adios_MPI_data_struct * md = (struct adios_MPI_data_struct *)
...
enum ADIOS_FLAG adios_mpi_nwu_should_buffer (struct adios_file_struct * fd
                                        ,struct adios_method_struct * method
                                        ,void * comm
                                        )
{
...
#include <unistd.h>
                ...
                md->striping_unit = adios_mpi_nwu_get_striping_unit(md->fh, name);
    ...
    if (fd->shared_buffer == adios_flag_no && fd->mode != adios_mode_read)
    {
        ...
        int count;
        count = adios_mpi_nwu_striping_unit_write(
                          md->fh,
                          fd->base_offset,
                          fd->buffer,
                          fd->bytes_written,
...
void adios_mpi_nwu_write (struct adios_file_struct * fd
...
void adios_mpi_nwu_get_write_buffer (struct adios_file_struct * fd
                                ,struct adios_var_struct * v
                                ,uint64_t * size
                                ,void ** buffer
                                ,struct adios_method_struct * method
...
void adios_mpi_nwu_read (struct adios_file_struct * fd
...
static void adios_mpi_nwu_do_read (struct adios_file_struct * fd
                              ,struct adios_method_struct * method
                              )
{
    struct adios_MPI_data_struct * md = (struct adios_MPI_data_struct *)
...
void adios_mpi_nwu_close (struct adios_file_struct * fd
                     ,struct adios_method_struct * method
                     )
{
    struct adios_MPI_data_struct * md = (struct adios_MPI_data_struct *)
    ...
    switch (fd->mode)
    {
        case adios_mode_read:
        {
            ...
            adios_mpi_nwu_do_read (fd, method);
            struct adios_var_struct * v = fd->group->vars;
            while (v)
            {
                v->data = 0;
        ...
        case adios_mode_write:
        {
            ...
            if (fd->shared_buffer == adios_flag_yes)
            {
                ...
                adios_mpi_nwu_striping_unit_write(
                                  md->fh,
                                  fd->base_offset,
                                  fd->buffer,
                                  fd->bytes_written,
...
void adios_mpi_nwu_finalize (int mype, struct adios_method_struct * method)
{
// nothing to do here
    if (adios_mpi_nwu_initialized)
        adios_mpi_nwu_initialized = 0;
}

void adios_mpi_nwu_end_iteration (struct adios_method_struct * method)
{
}

void adios_mpi_nwu_start_calculation (struct adios_method_struct * method)
{
}

void adios_mpi_nwu_stop_calculation (struct adios_method_struct * method)
{
}
