...
static ptlinfo *getConnectionInfo(MPI_Comm comm)
{
	int rank;
	int size;
...
struct ardma_client_connection * ardma_client_connect (MPI_Comm comm)
{
	...
	else
	{
		...
		remote = readConnectionFile("param");
	...
	client->self.id = local_info.pid;
	client->self.uid = local_info.uid;
	client->self.ac = local_info.ac;	
	client->self.match = remote->match;
