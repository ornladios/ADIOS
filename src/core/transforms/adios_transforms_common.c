...
////////////////////////////////////////
// Transform type name<->id conversion
////////////////////////////////////////
/*
 * Somewhat ugly construct for specifying a key->value mapping between
 * transform names (i.e., strings) and transform types (i.e., enum values)
 *
 * Multiple aliases are allowed for each type. When performing the inverse
 * mapping (type to name), the first matching alias is used.
 */
const struct {
    const char *alias;				// A possible name for a transform method
    enum ADIOS_TRANSFORM_TYPE type;	// The corresponding ADIOS_TRANSFORM_TYPE
} ADIOS_TRANSFORM_NAMES[] = {
     { "unknown"	, adios_transform_unknown }

    ,{ "none"		, adios_transform_none }
    ,{ "no"			, adios_transform_none }
    ,{ "raw"		, adios_transform_none }
...
/*
 * Converts a transform name into its transform type ID.
 * @param transform_name the transform name to convert.
 * @return the transform type ID, or adios_transform_unknown
 *         if no transform is found by the given name.
 */
enum ADIOS_TRANSFORM_TYPE adios_transform_type_by_name(const char *transform_name) {
    int i;
    for (i = 0; i < NUM_ADIOS_TRANSFORM_NAMES; i++) {
        if (strcasecmp(transform_name, ADIOS_TRANSFORM_NAMES[i].alias) == 0) {
            return ADIOS_TRANSFORM_NAMES[i].type;
        }
    }
...
/*
 * Converts a transform type ID to a string representation.
 * @param transform_type the transform type ID to convert.
 * @return a string representation of the given transform type.
 */
const char * adios_transform_name_by_type(enum ADIOS_TRANSFORM_TYPE transform_type) {
    int i;
    for (i = 0; i < NUM_ADIOS_TRANSFORM_NAMES; i++) {
        //printf("[TRANSFORM] Comparing transform name \"%s\" to alias \"%s\" for type %d\n", transform_name, ADIOS_TRANSFORM_NAMES[i].alias, ADIOS_TRANSFORM_NAMES[i].type);
        if (transform_type == ADIOS_TRANSFORM_NAMES[i].type) {
            //printf("[TRANSFORM] Transform name \"%s\" is type %d\n", transform_name, ADIOS_TRANSFORM_NAMES[i].type);
            return ADIOS_TRANSFORM_NAMES[i].alias;
        }
    }
    //printf("[TRANSFORM] Unrecognized transform name %s\n", transform_name);
    return "";
}
