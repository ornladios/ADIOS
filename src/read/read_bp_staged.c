...
*/
    ...
    if (isAggregator (p))
    {
        ...
        for (i = 0; i < fh->gvar_h->group_count; i++)
        {
            ...
            _buffer_write (&buffer, &buffer_size, &buffer_offset, 
                           &fh->gvar_h->var_counts_per_group[i], sizeof(uint64_t)); // var_counts_per_group
        }
        ...
        {
            _buffer_write (&buffer, &buffer_size, &buffer_offset, 
                           &fh->gattr_h->attr_counts_per_group[i], sizeof(uint64_t)); // attr_counts_per_group
        }
...
*/
    ...
    if (!isAggregator (p))
    {
        ...
        fh->gvar_h->var_counts_per_group = (uint64_t *) malloc (fh->gvar_h->group_count * sizeof(uint64_t));
        ...
        for (i = 0; i < fh->gvar_h->group_count; i++)
        {
            ...
            _buffer_read (buffer, &buffer_offset, &fh->gvar_h->var_counts_per_group[i], sizeof(uint64_t)); // var_counts_per_group
        }
        ...
        fh->gattr_h->attr_counts_per_group = (uint64_t *) malloc (fh->gattr_h->group_count * sizeof(uint64_t));
        fh->gattr_h->namelist = fh->gvar_h->namelist;

        for (i = 0; i < fh->gattr_h->group_count; i++)
        {
            _buffer_read (buffer, &buffer_offset, &fh->gattr_h->attr_counts_per_group[i], sizeof(uint64_t)); // attr_counts_per_group
        }
...
void adios_read_bp_staged_get_groupinfo (const ADIOS_FILE *fp, int *ngroups, char ***group_namelist, uint64_t **nvars_per_group, uint64_t **nattrs_per_group)
{
    BP_PROC * p;
    BP_FILE * fh;
    int i, j, k, offset;
