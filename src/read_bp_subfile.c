...
printf ("read subfile method\n");
    ...
    for (i=0;i<fp->groups_count;i++) {
        if (!fp->group_namelist[i]) {
            ...
            adios_read_bp_subfile_fclose(fp);
            return NULL;
        }
        else  {
...
ADIOS_GROUP * adios_read_bp_subfile_gopen (ADIOS_FILE *fp, const char * grpname)
{
    ...
    return adios_read_bp_subfile_gopen_byid(fp, grpid);
}
...
*/
static int adios_read_bp_subfile_get_dimensioncharacteristics(struct adios_index_characteristic_struct_v1 *ch, 
...
static void adios_read_bp_subfile_get_dimensions (struct adios_index_var_struct_v1 *var_root, int ntsteps, int file_is_fortran,
                                  int *ndim, uint64_t **dims, int *timedim)
{
    ...
    *dims = (uint64_t *) malloc (sizeof(uint64_t) * (*ndim));
    ...
    is_global = adios_read_bp_subfile_get_dimensioncharacteristics( &(var_root->characteristics[0]),
                                                    ldims, gdims, offsets);
...
int64_t adios_read_bp_subfile_read_var (ADIOS_GROUP * gp, const char * varname,
                        ...
                        void * data)
{
    ...
    return adios_read_bp_subfile_read_var_byid(gp, varid, start, count, data);
}
...
static double adios_stat_cor (ADIOS_VARINFO * vix, ADIOS_VARINFO * viy, char * characteristic, uint32_t time_start, uint32_t time_end, uint32_t lag)
{
    int i,j;
...
static double adios_stat_cov (ADIOS_VARINFO * vix, ADIOS_VARINFO * viy, char * characteristic, uint32_t time_start, uint32_t time_end, uint32_t lag)
{
    int i,j;
