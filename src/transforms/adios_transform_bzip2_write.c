...
{
    if(strlen(input_str) > 2)	// at most 2 digits for bzip2
    {
        return 0;
    }
...
uint64_t adios_transform_bzip2_calc_vars_transformed_size(struct adios_transform_spec *transform_spec, uint64_t orig_size, int num_vars)
{
    return (orig_size);
}
