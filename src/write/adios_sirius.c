...
int * allocOSTList (int n_ost)
{
    ...
    {   
        adios_error (err_no_memory, 
                "Can not malloc %d bytes in allocOSTList() in SIRIUS method\n",
                n_ost*sizeof(int));
        return 0;
    }
...
int * parseOSTSkipping (int * ost_list, char * str, int n_ost)
{
    ...
    if (ost_list == 0)
    {
        log_warn ("SIRIUS method: Pointer ost_list is null.\n");
        return 0;
    }
...
adios_sirius_set_striping_unit(struct adios_MPI_data_struct * md, char *parameters)
{
    ...
    if (fd != -1) {
        ...
        if (n_ost <= 0)
        {
            log_warn ("SIRIUS method: No OST to use. Set num_ost=NNN in the adios config xml file.\n");
            return;
        }
...
printf ("why is here\n");
    ...
    {
        log_warn("SIRIUS method: open to set lustre striping failed on file %s %s rank = %d.\n",filename,strerror(errno), md->rank);
    }
}
...
adios_sirius_set_aggregation_parameters(char * parameters, struct adios_MPI_data_struct * md)
{
    ...
    {
        adios_error (err_no_memory, "Can not malloc %d bytes in SIRIUS method, adios_sirius_set_aggregation_parameters()\n", nproc*sizeof(int));
        return;
    }
...
/*
static void adios_sirius_buffer_write (char ** buffer, uint64_t * buffer_size
                                       ...
                                       )
{
    if (*buffer_offset + size > *buffer_size || *buffer == 0)
    {
        ...
        {
            adios_error (err_no_memory, "Cannot allocate memory in adios_sirius_buffer_write.  "
...
static uint64_t
adios_sirius_striping_unit_write(MPI_File   fh
                                 ,MPI_Offset offset
                                 ,void       *buf
                                 ,uint64_t   len
                                 )
...
struct adios_var_struct * adios_sirius_copy_var (struct adios_var_struct * v)
{
    struct adios_var_struct * v_new = (struct adios_var_struct *) 
                            malloc (sizeof (struct adios_var_struct));
    if (v_new == 0)
    {
        adios_error (err_no_memory, "SIRIUS method: Cannot allocate %d bytes "
            "to duplicate variable structure in adios_sirius_copy_var()\n",
...
void adios_sirius_append_var (struct adios_file_struct * fd, struct adios_var_struct * v)
{
...
void adios_sirius_add_offset (uint64_t pg_offset_to_add,
...
void adios_sirius_subtract_offset (uint64_t var_offset_to_subtract
...
void adios_sirius_build_global_index_v1 (char * fname
                                         ,struct adios_index_struct_v1 * index
                                          )
{
    //struct adios_index_process_group_struct_v1 * pg_root = index->pg_root;
...
void * adios_sirius_do_mkdir (char * path)
{
    // 4 bytes for ".dir" 
...
void * adios_sirius_do_open_thread (void * param)
{
    ...
    if (td->parameters)
    {
        adios_sirius_set_striping_unit (td->md, td->parameters);
    ...
    if (err != MPI_SUCCESS)
    {
        ...
        adios_error (err_file_open_error,
                     "SIRIUS method: MPI open failed for %s: '%s'\n",
...
void * adios_sirius_do_write_thread (void * param)
{
    struct adios_MPI_thread_data_write * td = (struct adios_MPI_thread_data_write *) param;

    uint64_t count = adios_sirius_striping_unit_write(
                               *(td->fh)
                              ,*(td->base_offset)
                              ,td->aggr_buff
                              ,*(td->total_data_size)
    ...
    {
        adios_error (err_unspecified, "Error in adios_sirius_striping_unit_write(). "
    ...
    switch (fd->mode)
    {
        ...
        case adios_mode_write:
        {
            if (md->rank == 0)
            {
                ...
                adios_sirius_set_have_mdf (method->parameters, md);
                if (md->g_have_mdf)
                {
                    f = open(name, O_CREAT | O_RDWR | O_LOV_DELAY_CREATE, 0644);
                    if (f == -1)
                    {
//                        adios_error (err_file_open_error,"SIRIUS method: open() failed: %s\n", strerror(errno));
                        adios_error (err_file_open_error,"SIRIUS method: open() failed: %s\n", name);
                        return -1;
                    }
                    ...
                    if (rc != 0)
                    {
                        log_warn ("SIRIUS method: Lustre get uuids failed after creating the file: %s\n" ,
                                  strerror(errno));
                    }
...
//                adios_sirius_do_mkdir (name);
            }

            MPI_Bcast (&md->g_num_ost, 1, MPI_INT, 0, md->group_comm);
          
            adios_sirius_set_aggregation_parameters (method->parameters, md);
            ...
            if (is_aggregator(md->rank))
            {
                ...
                if (md->g_threading)
                {
                    pthread_create (&md->g_sot, NULL
                            ,adios_sirius_do_open_thread
                            ,(void *) md->open_thread_data
                            );
                }
                else
                {
                    adios_sirius_do_open_thread ((void *) md->open_thread_data);
                }
            }
        ...
        case adios_mode_update:
        {
            ...
            if (md->rank == 0)
            {
                ...
                if (md->g_have_mdf)
                {
                    ...
                    if (f == -1)
                    {
                        adios_error (err_file_open_error,"SIRIUS method: open() failed at append: %s\n", name);
                        return -1;
                    }
                    ...
                    if (rc != 0)
                    {
                        log_warn ("SIRIUS method: Lustre get uuids failed after opening the file: %s\n" ,
                                  strerror(errno));
                    }
            ...
            adios_sirius_set_aggregation_parameters (method->parameters, md); // is_aggregator() works after this
            ...
            if (is_aggregator(md->rank))
            {
                 ...
                 */
                {
                    ...
                    adios_sirius_do_reopen_thread ((void *) md->reopen_thread_data);
                }
                                ...
                                )
{
    ...
    if (mem_allowed == *size)
    {
        *buffer = malloc (*size);
        if (!*buffer)
        {
            ...
            adios_error (err_no_memory, 
                    "SIRIUS method: Out of memory allocating %llu bytes for variable %s\n",
                    *size ,v->name);
    ...
    else
    {
        ...
        adios_error (err_buffer_overflow, 
                "SIRIUS method: OVERFLOW: Cannot allocate requested buffer of %llu "
    ...
    switch (fd->mode)
    {
        case adios_mode_read:
        {
            adios_error (err_invalid_file_mode, 
                    "Only \"w\" mode is supported by SIRIUS Brigade IO\n");
            break;
        }
        case adios_mode_update:
        case adios_mode_append:
        case adios_mode_write:
        {
            ...
            if (!md->g_merging_pgs)
            {
                ...
                if (is_aggregator (md->rank))
                {
                    ...
                    {
                        adios_error (err_no_memory, "SIRIUS method (with brigade strategy): Cannot allocate "
            ...
            else 
            {
                ...
                log_warn ("SIRIUS method (BG): Merging process blocks is not supported yet\n");
            }
