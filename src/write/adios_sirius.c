...
static void output_vars(struct aggr_var_struct *vars, int varcnt, struct
        adios_sirius_data_struct * md, struct adios_file_struct * fd)
{
    int i;
    char file_mode[2];
    uint64_t adios_size;
...
static void init_layout_flag(struct adios_sirius_data_struct *md)
{
    int i;
    ...
    while (p) {
        if (!strcasecmp (p->name, "chunk_size")) {
            ...
            if (aggr_chunksize > 0 && !errno) {
                log_debug ("Chunk size set to %d for SIRIUS method\n", aggr_chunksize);
            } else {
                log_error ("Invalid 'chunk_size' parameter given to the SIRIUS method"
                           "method: '%s'\n", p->value);
            ...
            if (!errno) {
                log_debug ("io_method set to %s for SIRIUS method\n", io_method);
            } else {
                log_error ("Invalid 'io_method' parameter given to the SIRIUS method: '%s'\n", p->value);
                memset(io_method, 0x00, 16);
            ...
            if (!errno) {
                log_debug ("io_parameters set to %s for SIRIUS method\n", io_parameters);
            } else {
                log_error ("Invalid 'io_parameters' parameter given to the SIRIUS"
                            "method: '%s'\n", p->value);
                memset(io_parameters, 0x00, 256);
            }
        } else {
...

{
    ...
    if(ndims) //multidimensional data
    {
        ...
        if(ndims<=3 && varsize<=aggr_chunksize && md->size>1 && vars->set_aggr==-1) {
            ...
            log_error("Current SIRIUS only supports up to 3-D variables with minimum of 2 processes with 1D decomposition. No spatial merging will be performed.\n");
