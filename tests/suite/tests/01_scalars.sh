...
$TRUNKDIR/utils/bpls/bpls -lav scalars.bp > c_bpls.txt
...
if [ $? != 0 ]; then
    ...
    echo "Compare bpls -lav $PWD/scalars.bp to reference $SRCDIR/reference/scalars_write_bpls.txt"
    exit 1
fi
...
echo "Check output with bpls"
$TRUNKDIR/utils/bpls/bpls -lav scalars.bp > f_bpls.txt
...
if [ $? != 0 ]; then
    ...
    echo "Compare bpls -lav scalars.bp to reference $SRCDIR/reference/scalars_write_bpls.txt"
    exit 1
fi
