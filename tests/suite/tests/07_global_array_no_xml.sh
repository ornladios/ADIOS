...
$TRUNKDIR/utils/bpls/bpls -lav adios_global_no_xml.bp -d -n 10 > c_bpls.txt
...
if [ $? != 0 ]; then
    ...
    echo "Compare \"bpls -lav $PWD/adios_global_no_xml.bp -d -n 10\" to reference $SRCDIR/reference/global_array_no_xml_bpls.txt"
    exit 1
fi

echo "Run C adios_read_global_no_xml"
$MPIRUN $MPIRUN_NP $READPROCS ./adios_read_global_no_xml | sort > c_read.txt
EX=$?
if [ $? != 0 ]; then
...
echo "Check output with bpls"
$TRUNKDIR/utils/bpls/bpls -lav adios_global.bp -d -n 10 > f_bpls.txt
...
if [ $? != 0 ]; then
    ...
    echo "Compare \"bpls -lav $PWD/adios_global.bp -d -n 10\" to reference $SRCDIR/reference/global_array_bpls.txt"
    exit 1
fi
