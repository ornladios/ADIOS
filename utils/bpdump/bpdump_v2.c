...
void print_process_group_header_v2 (uint64_t num
                      ,struct adios_process_group_header_struct_v2 * pg_header
                      );
void print_vars_header_v2 (struct adios_vars_header_struct_v2 * vars_header);
void print_var_header_v2 (struct adios_var_header_struct_v2 * var_header);
void print_var_payload_v2(struct adios_var_header_struct_v2 * var_header
                       ,struct adios_var_payload_struct_v2 * var_payload
                       ,struct dump_struct * dump
                       ,int var_dims_count
                       ,struct var_dim * var_dims
                       );
void print_attrs_header_v2 (
                      struct adios_attributes_header_struct_v2 * attrs_header
                      );
void print_attribute_v2 (struct adios_attribute_struct_v2 * attribute);
void print_process_group_index_v2 (
                         struct adios_index_process_group_struct_v2 * pg_root
                         );
void print_vars_index_v2 (struct adios_index_var_struct_v2 * vars_root);
void print_attributes_index_v2
...
int main (int argc, char ** argv)
{
    ...
    struct adios_bp_buffer_struct_v2 * b = 0;
    uint32_t version = 0;

    b = malloc (sizeof (struct adios_bp_buffer_struct_v2));
    adios_buffer_struct_init_v2 (b);

    rc = adios_posix_open_read_internal_v2 (filename, "", b);
    if (!rc)
    {
    ...
    adios_posix_read_version_v2 (b);
    adios_parse_version_v2 (b, &version);

    struct adios_index_process_group_struct_v2 * pg_root = 0;
    struct adios_index_process_group_struct_v2 * pg = 0;
    struct adios_index_var_struct_v2 * vars_root = 0;
    struct adios_index_attribute_struct_v2 * attrs_root = 0;

    printf (DIVIDER);
    printf ("Process Groups Index:\n");
    adios_posix_read_index_offsets_v2 (b);
    adios_parse_index_offsets_v2 (b);
    ...
    */
    ...
    adios_posix_read_process_group_index_v2 (b);
    adios_parse_process_group_index_v2 (b, &pg_root);
    print_process_group_index_v2 (pg_root);

    printf (DIVIDER);
    printf ("Vars Index:\n");
    adios_posix_read_vars_index_v2 (b);
    adios_parse_vars_index_v2 (b, &vars_root);
    print_vars_index_v2 (vars_root);

    printf (DIVIDER);
    printf ("Attributes Index:\n");
    adios_posix_read_attributes_index_v2 (b);
    adios_parse_attributes_index_v2 (b, &attrs_root);
    print_attributes_index_v2 (attrs_root);
    ...
    while (pg)
    {
        ...
        for (i = 0; i < vars_header.count; i++)
        {
            ...
            adios_parse_var_data_header_v2 (b, &var_header);
            print_var_header_v2 (&var_header);
               ...
               )
            {
                ...
                adios_parse_var_data_payload_v2 (b, &var_header, &var_payload
                                                ,var_header.payload_size
                                                );
            }
            else
            {
                adios_parse_var_data_payload_v2 (b, &var_header, NULL, 0);
            }
            ...
            if (dump.do_dump)
            {
                ...
                print_var_payload_v2 (&var_header, &var_payload, &dump
        ...
        adios_parse_attributes_header_v2 (b, &attrs_header);
        print_attrs_header_v2 (&attrs_header);

        for (i = 0; i < attrs_header.count; i++)
        {
            adios_parse_attribute_v2 (b, &attribute);
            print_attribute_v2 (&attribute);
            printf ("\n");
        }
    ...
    adios_posix_close_internal_v2 (b);
...
{
    int i;
    struct adios_method_info_struct_v2 * m;
...
void print_var_header_v2 (struct adios_var_header_struct_v2 * var_header)
{
    ...
    if (var_header->dims)
    {
        struct adios_dimension_struct_v2 * d = var_header->dims;
        printf ("\t\tDimensions:\n");
        while (d)
        {
