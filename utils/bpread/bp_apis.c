...
//			printf("\tndim:%d\n",*ndim);
			...
			for (j=0;j<var_root->characteristics_count;j++) {
				for (k=0;k<var_root->characteristics [j].dims.count;k++) {
					if(dims[k]<var_root->characteristics[j].dims.dims[k*3+1]) {
						dims[k]=var_root->characteristics[j].dims.dims[k*3+1];
					}
				}
			}
		...
		)
{
	...
	for (idx = 0; idx < count; idx++) {
		...
		int size_of_type = adios_get_type_size (var_root->type, "");
